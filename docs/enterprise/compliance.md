...
By default everything lives under `~/.memlayer/` (override with
`MEMLAYER_DATA_DIR`). Contents:
...
| `config.toml` / `config.json` | Global config | Written by `memlayer install` and `memlayer config`. |
...
result count, duration. Set `MEMLAYER_AUDIT_FULL=1` to additionally carry the
...
- `MEMLAYER_AUDIT_FULL=1` is **opt-in**; default rows avoid leaking query
...
- Embeddings / indexes are rebuilt on save / `memlayer reindex`; no periodic
  GC job runs by default.
- Logs rotate as described above only.

Deletion commands (all explicit — memlayer never silently GCs):
...
| `memlayer clean` | **Wipes all stored observations and stops the daemon.** |
| `memlayer project prune` | Lists projects with zero active observations (dry-run flag for review). |
...
- **Local-first.** Default deployment stores all memory on the machine that
  runs the daemon (`~/.memlayer/`). No telemetry upload, no account, no cloud
  dependency for retrieval.
- **No compliance certifications are claimed.** memlayer has not submitted to
...
- **Third-party data flow (LLM steps only, opt-in).** When `extract`,
  ...
  `PATH`, memlayer pipes prompt text to that CLI (shelled out, stdin/argv).
  ...
  **off by default** (`extract.enabled=false`, `conflict.enabled=true` after
  `memlayer install`, `rerank=false`). Hybrid search itself (BM25 + local
...
- **At-rest encryption.** SQLite files are plaintext. Two encryption options:
  ...
  2. BYOK on export: `mem export --seed-phrase` / `--seed-file` encrypts
     ...
     seed is never stored by memlayer — losing it loses the archive.
...
| TCP transport | TLS mandatory (`MEMLAYER_LISTEN=tcp://…` aborts startup without `MEMLAYER_TLS_CERT` / `MEMLAYER_TLS_KEY`). rustls (TLS 1.2/1.3). |
| Bearer tokens | `tokens.db` holds token names + admin flag. Admin gates: `Shutdown`, `token-*`, `project delete --hard`. |
| Per-project grants | Managed via `memlayer team grant` — principal → role (`read`/`write`) per project. Opt-in: projects with no grants are open to any valid token; once gated, nongranted principals are denied and `read` rejects writes. |
| Client validation | Client library pins the daemon's `ca.pem` (from `team init-ca`) and presents the bearer token over TLS. **Scope note:** the daemon side (TLS, token store, admin guard, grants) is implemented; today the CLI's own transport is UDS, TCP access rides the `memlayer-client` library surface. |
| Auditable / integrity | `memlayer doctor` runs DB integrity audit; `memlayer verify` re-checks code anchors vs git history (wired to post-commit / post-merge / post-checkout hooks). |

## Map to common questionnaire items

| Question | Answer |
|---|---|
| Where is data stored? | On the daemon host under `~/.memlayer/` (or `MEMLAYER_DATA_DIR`). |
...
| Can we delete our data? | Yes — `obs delete --hard`, `project delete --hard`, or `memlayer clean`. No retention locks. |
