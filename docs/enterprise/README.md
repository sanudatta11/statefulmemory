# statefulmemory Enterprise docs

Operational and procurement material for running statefulmemory on infrastructure
you control.
...
| [compliance.md](compliance.md) | Data map (`~/.statefulmemory/**`), retention + deletion, audit trail (`queries.log`), compliance posture statement (local-first; **no certifications claimed**), enterprise security surface. |
| [cloud.md](cloud.md) | Honest status of StatefulMemory Cloud: waitlist + regions TBD, no invented SLAs; what self-host covers today vs. what Cloud would add. |
...
- All facts are stated against the current codebase (CLI verbs, schema,
  env vars). Recheck with `statefulmemory team --help`, `statefulmemory daemon --help`,
  and the crate source before quoting these docs in a security review.
- `statefulmemory daemon start --foreground` is the foreground form used by the
