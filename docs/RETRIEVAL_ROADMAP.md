...
| Manual eval workflow | `eval.yml` is `workflow_dispatch` only (dataset: limit50 / full / smoke); README scorecard block updates only when `update_readme=true` | No CI commit spam; published cards stay LLM-run owned |
...
| Cost | Magnitude | Mitigation |
|---|---|---|
| CI compute | LoCoMo limit50 lexical ≈ minutes; full LLM run hours locally | `workflow_dispatch` + actions/cache; README update is opt-in |
...
2. CI job (`.github/workflows/eval.yml`) — manual `workflow_dispatch`
   (dataset input: limit50 / full / smoke); optional README scorecard
   update when `update_readme=true`.
3. Scorecard publisher — `.github/scripts/update-readme-scorecard.mjs`
   rewrites the README block from the scorecard JSON.
...
- **Eval gameability.** A change can lift LoCoMo while regressing real
  ...
  set on a schedule (not every PR — eval is `workflow_dispatch` only) to catch this.
