...
use statefulmemory_eval::config::{default_profile, RetrievalMode};
use statefulmemory_eval::rerank::{build_rerank_prompt, parse_rerank_indices};
use statefulmemory_eval::runner::BenchmarkKind;
...
fn ts11_beam1m_disables_rerank() {
    ...
    assert!(
        !p.rerank,
        "BEAM-1M should disable rerank (LLM cost not justified)"
    );
