...
use statefulmemory_embed::quantize::{calibrate_scale, f32_to_int8, int8_to_f32};
...
fn ts12_int8_topk_jaccard_at_least_0_9() {
    ...
    let corpus_int8: Vec<Vec<i8>> = corpus.iter().map(|v| f32_to_int8(v, scale)).collect();
    let corpus_recovered: Vec<Vec<f32>> =
        corpus_int8.iter().map(|v| int8_to_f32(v, scale)).collect();

    let mut total_j = 0.0_f32;
    for q in &queries {
        let scores_f32: Vec<f32> = corpus.iter().map(|c| cosine(q, c)).collect();
        let scores_int8: Vec<f32> = corpus_recovered.iter().map(|c| cosine(q, c)).collect();
