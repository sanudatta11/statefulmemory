...
use statefulmemory_eval::datasets::EvalMemory;
use statefulmemory_eval::entities_writer::{bulk_upsert_entities, EntityRow};
use statefulmemory_eval::facts_db::FactsDb;
use statefulmemory_eval::ingest::ingest_memories;
use statefulmemory_eval::retrieve_facts::retrieve_facts;
...
async fn ts21_entity_name_match_boosts_linked_fact_above_unlinked() {
    ...
    let mems = vec![
        mk_mem(
            ...
            &session_id,
            "Caroline trip",
            "Caroline went to Paris last summer",
        ),
        mk_mem(
            project,
            &session_id,
            "Melanie hobby",
            "Melanie enjoys painting on weekends",
        ),
        mk_mem(
            project,
            &session_id,
    ...
    let f1_id = insert_fact(
        &facts_db, project, obs_ids[0], "Caroline", "went-to", "Paris",
    );
    let f2_id = insert_fact(
        &facts_db, project, obs_ids[1], "Melanie", "enjoys", "painting",
    );
