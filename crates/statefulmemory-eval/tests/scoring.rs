...
fn build_score_map_combines_three_sources() {
    let bm25_hits = vec![(1, -2.0_f64), (3, -3.0_f64), (5, -1.0_f64)];
    let dense_hits = vec![(2, 0.4_f64), (3, 0.1_f64), (6, 0.7_f64)];
    ...
    let mut ranked: Vec<(i64, f32)> = map.iter().map(|(id, sc)| (*id, sc.combined())).collect();
    ranked.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    assert_eq!(
        ranked[0].0, 3,
        "3-signal fact must rank first; got {ranked:?}"
    );
