...
use statefulmemory_eval::{datasets::EvalMemory, ingest::ingest_memories, retrieve::retrieve};

/// `STATEFULMEMORY_DATA_DIR` is process-global; serialize these tests.
...
async fn single_keyword_query_finds_matching_memory() {
    ...
    let result = retrieve(&data_dir, "test-proj", "Caroline", 10).expect("retrieve should succeed");
...
async fn stopword_only_query_does_not_panic() {
    ...
    assert!(
        result.hits.is_empty() || !result.hits.is_empty(),
...
async fn project_isolation_no_cross_project_leakage() {
    ...
    let session = uuid::Uuid::new_v4().to_string();
    let mems_a = vec![mk_mem(
        "proj-a",
        &session,
        "alpha note",
        "Caroline visited the museum in Paris.",
    )];
    let mems_b = vec![mk_mem(
        "proj-b",
        &session,
        "beta note",
...
async fn k_limits_returned_results() {
    ...
    assert!(
        result.hits.len() <= 5,
        "k=5 must cap returned hits; got {}",
        result.hits.len()
    );
...
async fn ranking_prefers_more_specific_match() {
    ...
    let result = retrieve(&data_dir, "rank-proj", "Caroline LGBTQ support group", 10).unwrap();
