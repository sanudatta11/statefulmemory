...
async fn phrase_fallback_returns_hits_on_multi_word_query() {
    ...
    let hit_scattered = result.hits.iter().any(|h| h.contains("supports stuff"));
    assert!(
        hit_phrase,
...
async fn single_token_query_works_unchanged() {
    ...
    let result =
        retrieve(&data_dir, "single-proj", "Caroline", 10).expect("retrieve should succeed");

    assert!(
        !result.hits.is_empty(),
        "expected hits for single-token query"
    );
    assert!(
