...
use statefulmemory_extract::cache::{CachedExtraction, ExtractionCache};
use statefulmemory_extract::claude_cli::{ClaudeClient, HAIKU_MODEL};
use statefulmemory_extract::entities::EntityExtractor;
use statefulmemory_extract::{build_extraction_prompt, parse_facts, Fact, Turn};
...
impl ExtractPipeline {
    ...
    pub async fn extract_project(
        ...
        // ---------- 1. Open targets ----------
        // Mirror retrieve.rs: storage layer reads STATEFULMEMORY_DATA_DIR through
        // statefulmemory-core::paths, so set it before the registry opens.
        std::env::set_var("STATEFULMEMORY_DATA_DIR", &self.data_dir);
        ...
        let extraction_cache =
            ExtractionCache::open(&self.data_dir).context("open extraction cache")?;
        let embedding_cache =
            EmbeddingCache::open(&self.data_dir).context("open embedding cache")?;
        ...
        if miss_indices.is_empty() {
            ...
            // bubble as Err (caller fails the extract step; never cache them).
            {
                ...
                info!(
                    project,
                    window = probe_idx,
                    "probe: testing first window before fan-out"
                );
                let probe_raw = self
                    .claude
                ...
                info!(
                    project,
                    window = probe_idx,
                    facts = probe_facts.len(),
                    "probe ok — proceeding with fan-out"
                );
                // Persist probe result now; fan-out loop skips index 0.
                extraction_cache
                    .put_many(&[(window_keys[probe_idx].clone(), probe_facts)])
        ...
        if all_facts.is_empty() {
            stats.elapsed_ms = t0.elapsed().as_millis();
            info!(project, ?stats, "extract_project: no facts extracted");
            return Ok(stats);
        }
        ...
        if !emb_miss_indices.is_empty() {
            let miss_texts: Vec<&str> = emb_miss_indices.iter().map(|&i| text_refs[i]).collect();
            let fresh = self
                .embedder
                .embed(&miss_texts)
        ...
        let fact_ids: Vec<i64> = if let (true, Some(shard_dir)) =
            (self.shards > 1, self.shard_dir.as_ref())
        {
            ...
            for (shard_idx, indices) in by_shard {
                ...
                let mut shard_db = FactsDb::open(&shard_path)
                    .with_context(|| format!("open shard at {}", shard_path.display()))?;
                let sub_batch: Vec<FactWithEmbedding> =
                    indices.iter().map(|&i| batch[i].clone()).collect();
                let sub_ids =
                    insert_facts_for_project_returning_ids(&mut shard_db.conn, project, &sub_batch)
        ...
        if self.shards <= 1 && self.session_summaries && !batch.is_empty() {
            ...
            .await
            {
                Ok(written) => {
                    info!(
                        project,
                        session_summaries = written,
                        "wrote session-tier facts"
                    );
...
async fn extract_and_write_entities(
    ...
    for (fwe, &fact_id) in batch.iter().zip(fact_ids.iter()) {
        ...
        for name in names {
            ...
            entity_to_fact_ids.entry(name).or_default().push(fact_id);
        }
    }
    ...
    if !miss_indices.is_empty() {
        ...
        let fresh = embedder.embed(&miss_texts).context("embed entity names")?;
        if fresh.len() != miss_texts.len() {
            anyhow::bail!(
    ...
    let written = bulk_upsert_entities(conn, project, &rows).context("bulk upsert entities")?;
    Ok(written)
}
...
async fn write_session_summaries(
    ...
    for (session, facts) in by_session {
        ...
        top.truncate(5);
        let mean_salience: f32 =
            top.iter().map(|f| f.fact.salience).sum::<f32>() / top.len() as f32;
        ...
        let context: String = top
            ...
            .map(|f| {
                ...
                format!(
                    "- {}{} {} {}",
                    temp, f.fact.subject, f.fact.predicate, f.fact.object
                )
            })
            .collect::<Vec<_>>()
            .join("\n");
        ...
        let summary = match claude
            .ask(&prompt, statefulmemory_extract::claude_cli::HAIKU_MODEL)
            .await
        {
...
fn embed_summary(
    ...
    let (cached, misses) = cache.get_many(&[text]).context("read embedding cache")?;
    if misses.is_empty() {
        if let Some(v) = cached.into_iter().next().flatten() {
            return Ok(v);
        }
    }
    let mut vs = embedder.embed(&[text]).context("embed session summary")?;
    let v = vs.remove(0);
    cache
