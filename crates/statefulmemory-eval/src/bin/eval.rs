...
//! `eval` binary — CLI entry point for statefulmemory benchmark runs.
//!
//! Usage:
...
#[derive(Parser)]
#[command(name = "eval", about = "statefulmemory benchmark runner")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
    ...
    match cli.command {
        Commands::Run {
            ...
            let (memories, queries) =
                load_dataset(benchmark, &data_dir, limit.unwrap_or(usize::MAX))?;
            ...
            let cfg = RunConfig {
                ...
                k,
                limit,
                concurrency: statefulmemory_eval::runner::eval_concurrency_from_env(4),
                output_path: out,
                skip_ingest,
                retrieval,
                trace_path,
            ...
            let report = statefulmemory_eval::runner::run(&cfg, memories, queries).await?;
...
fn load_dataset(
    ...
    Vec<statefulmemory_eval::datasets::EvalMemory>,
    Vec<statefulmemory_eval::datasets::EvalQuery>,
)> {
    match kind {
        ...
        BenchmarkKind::Beam1m => {
            let (m, q) =
                statefulmemory_eval::datasets::beam::generate_all(BeamScale::M1, limit.min(1000))?;
            Ok((m, q))
        }
...
async fn run_extract(
    ...
    use statefulmemory_embed::BgeSmallEmbedder;
    use statefulmemory_eval::extract_pipeline::ExtractPipeline;
    use statefulmemory_extract::claude_cli::ClaudeCliClient;
    use statefulmemory_extract::entities::{HaikuEntityExtractor, HeuristicEntityExtractor};
    use std::sync::Arc;
    ...
    if memories.is_empty() {
        println!(
            "No memories loaded for {:?}; nothing to extract.",
            benchmark
        );
        return Ok(());
    }
    ...
    println!(
        "Ingesting {} observations into storage projects...",
        memories.len()
    );
    statefulmemory_eval::ingest::ingest_memories(data_dir, &memories, 500, false).await?;

    let facts_db_path = statefulmemory_eval::runner::facts_db_path_for(benchmark, data_dir);
    println!("Facts DB: {}", facts_db_path.display());

    let claude = Arc::new(ClaudeCliClient::new());
    let embedder = Arc::new(
        BgeSmallEmbedder::try_new().map_err(|e| anyhow::anyhow!("load BGE-small embedder: {e}"))?,
    );
    ...
    let all_projects: Vec<String> = memories
        .iter()
        .filter_map(|m| {
            if seen.insert(m.project.clone()) {
                Some(m.project.clone())
            } else {
                None
            }
        })
...
fn summarize_reports(reports_dir: &PathBuf, out: &PathBuf) -> Result<()> {
    use statefulmemory_eval::runner::RunReport;
    ...
    let mut md = String::from(
        "# statefulmemory Benchmark Summary\n\n\
