...
fn retrieve_hybrid_sync(
    ...
    let hits = fetch_observations(registry, project, &top_k).context("fetch observations by id")?;
...
fn ann_top_n(data_dir: &Path, project: &str, query_vec: &[f32], n: i32) -> Result<Vec<u64>> {
...
fn ensure_vec_index(
    ...
    let texts: Vec<String> = rows.iter().map(|(_, t, c)| format!("{t} {c}")).collect();
    let text_refs: Vec<&str> = texts.iter().map(|s| s.as_str()).collect();
    let (mut hit_vecs, miss_indices) = cache
        .get_many(&text_refs)
        .context("cache get_many for obs")?;
    ...
    {
        let mut ins_vec = tx
            .prepare("INSERT INTO observations_vec(rowid, embedding) VALUES (?1, ?2)")
            ...
            rows.iter().zip(hit_vecs.iter()).enumerate()
        {
            ...
            ins_map.execute(params![rowid, obs_id]).with_context(|| {
