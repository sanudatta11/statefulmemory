...
pub fn compute_entity_walk_boost(
    ...
    let conn = open_with_vec(facts_db_path).context("open facts.db for entity walk")?;
    ...
    let sql = format!("SELECT id FROM entities WHERE project = ?1 AND name IN ({placeholders})");
    ...
    let mut hop1_facts: HashSet<i64> = HashSet::new();
    {
        ...
        let id_params: Vec<SqlValue> = q_entity_ids.iter().map(|i| SqlValue::Integer(*i)).collect();
    ...
    let mut co_entity_ids: HashSet<i64> = HashSet::new();
    {
        ...
        let id_params: Vec<SqlValue> = hop1_facts.iter().map(|i| SqlValue::Integer(*i)).collect();
    ...
    {
        let mut stmt = conn
            .prepare("SELECT fact_id FROM entity_links WHERE entity_id = ?1 LIMIT ?2")
