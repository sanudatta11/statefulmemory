...
pub fn insert_facts(conn: &mut Connection, batch: &[FactWithEmbedding]) -> Result<usize> {
...

    {
        ...
        for fwe in batch {
            ...
            let project = f.source_session.as_deref().unwrap_or("default");
            ins_fact
                .execute(params![
                    project,
                    f.evidence_obs_id,
            ...
            let bytes: Vec<u8> = fwe.embedding.iter().flat_map(|x| x.to_le_bytes()).collect();
            ins_vec
...
/// stays an internal detail of statefulmemory-extract.
...
pub fn insert_facts_for_project_returning_ids(
    ...
    // P4 spec-task-24: env-gated int8 path. When STATEFULMEMORY_VEC_INT8=1
    ...
    let int8_mode = std::env::var("STATEFULMEMORY_VEC_INT8")
        .map(|v| v == "1")
        .unwrap_or(false);
        ...
        for fwe in batch {
            ...
            let rowid: i64 = if inserted {
                ...
                match (&mut ins_vec_int8, int8_scale) {
                    ...
                    _ => {
                        let bytes: Vec<u8> =
                            fwe.embedding.iter().flat_map(|x| x.to_le_bytes()).collect();
                        ins_vec_f32
