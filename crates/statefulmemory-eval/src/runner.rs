...
pub struct RunConfig {
    ...
    /// appears in those hits (case-insensitive). Used by `statefulmemory eval
...
pub async fn run(
    ...
    let judge = if cfg.lexical_judge {
        ...
        if let Some(p) = statefulmemory_extract::agent_cli::detect_provider() {
            ui.note(format!(
                "LLM provider={} bin={} (override with STATEFULMEMORY_LLM_PROVIDER / STATEFULMEMORY_LLM_BIN)",
                p.id, p.bin
            ));
        } else {
    ...
    let hybrid_stack: Option<(
        std::sync::Arc<dyn statefulmemory_embed::Embedder>,
        std::sync::Arc<statefulmemory_embed::cache::EmbeddingCache>,
    ...
    ui.note(format!(
        "query concurrency={concurrency} (STATEFULMEMORY_EVAL_CONCURRENCY)"
    ));

    // Optional per-query trace writer (Mutex — concurrent queries).
    let trace_writer: Option<
    ...
    if results.is_empty() && !cfg.lexical_judge && total_queries > 0 {
        ...
        anyhow::bail!(
            "all {total_queries} queries skipped after answer LLM failures \
             ...
             export STATEFULMEMORY_LLM_PROVIDER=opencode  # or gemini / claude"
        );
    }
...
async fn eval_one_query(
    ...
    rerank_claude: Option<&std::sync::Arc<dyn statefulmemory_extract::claude_cli::ClaudeClient>>,
    trace_writer: Option<&std::sync::Arc<tokio::sync::Mutex<std::io::BufWriter<std::fs::File>>>>,
...
fn hit_contains_dia_id(haystack: &str, needle: &str) -> bool {
    ...
    h.contains(&format!("[{needle}]")) || h.contains(&format!("({needle})")) || h.contains(needle)
}
...
fn gold_substring_rank(gold: &str, hits: &[String]) -> Option<usize> {
    ...
    hits.iter()
        .position(|h| h.to_ascii_lowercase().contains(&g))
}
...
fn build_report(
    ...
    for r in &results {
        let Some(cat) = r.category.as_ref() else {
            continue;
        };
    ...
    let has_anti = results
        .iter()
        .any(|r| r.category.as_deref() == Some("staleness"));
    ...
    let retrieval_p50 = percentile_ms(
        &mut results.iter().map(|r| r.retrieval_us).collect::<Vec<_>>(),
        50,
    );
    let retrieval_p95 = percentile_ms(
        &mut results.iter().map(|r| r.retrieval_us).collect::<Vec<_>>(),
        95,
    );
    let e2e_p50 = percentile_ms(
        &mut results.iter().map(|r| r.end_to_end_us).collect::<Vec<_>>(),
        50,
    );
    let e2e_p95 = percentile_ms(
        &mut results.iter().map(|r| r.end_to_end_us).collect::<Vec<_>>(),
        95,
    );
