...
/// EntityExtractor in statefulmemory-extract handles that contract).
...
pub fn link_entity_to_facts(conn: &Connection, entity_id: i64, fact_ids: &[i64]) -> Result<usize> {
...
pub fn upsert_entity_vec(conn: &Connection, entity_id: i64, embedding: &[f32]) -> Result<()> {
