...
//! `statefulmemory-storage` and the project DB stay pristine (SC-10).
...
pub fn open_with_vec(path: &Path) -> Result<Connection> {
    if let Some(parent) = path.parent() {
        if !parent.as_os_str().is_empty() {
            std::fs::create_dir_all(parent).with_context(|| {
                format!(
                    "create parent directory for vec sidecar: {}",
                    parent.display()
                )
            })?;
        }
    ...
    let conn =
        Connection::open(path).with_context(|| format!("open sqlite db at {}", path.display()))?;
    ...
    tracing::debug!(
        target: "statefulmemory_eval::vec_index",
        version = %version,
        path = %path.display(),
...
pub fn bootstrap_vec_index(conn: &Connection, table_prefix: &str, dim: usize) -> Result<()> {
