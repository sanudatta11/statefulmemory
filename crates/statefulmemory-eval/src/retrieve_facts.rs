...
//!
//! SC-10: this module never touches `statefulmemory-storage` migrations or schema.
...
pub async fn retrieve_facts(
    ...
    // 1. Embed the query.
    let query_vec = embed_with_cache(query, &embedder, &cache).context("embed query")?;
    ...
    let mut query_entity_vecs: Vec<(String, Vec<f32>)> = Vec::with_capacity(query_entities.len());
    ...
    let bm25_handle =
        tokio::task::spawn_blocking(move || bm25_top_n(&bm25_path, &bm25_proj, &bm25_query, n));

    let ann_path = facts_db_path.to_path_buf();
    let ann_proj = project.to_string();
    let ann_query_vec = query_vec.clone();
    let ann_handle =
        tokio::task::spawn_blocking(move || ann_top_n(&ann_path, &ann_proj, &ann_query_vec, n));
    ...
    let candidates_considered = bm25_hits.len() + dense_hits.len() + entity_boost_map.len();
    ...
    if dense_hits.is_empty() {
        tracing::warn!(
            target: "statefulmemory_eval::retrieve_facts",
            project = %project,
    ...
    let walk_boosts =
        crate::entity_walk::compute_entity_walk_boost(facts_db_path, project, &query_entities)
    ...
    let meta_by_id =
        fetch_meta_for_ids(facts_db_path, &scored_ids).context("fetch fact meta for scoring")?;
    ...
    let mut ranked: Vec<(i64, f32)> = scored.iter().map(|(id, sc)| (*id, sc.combined())).collect();
    ...
    for (id, subject, predicate, object, temporal, evidence_obs_id) in fetched_facts {
        ...
        let evidence =
            match crate::retrieve::expand_evidence(&storage_conn, evidence_obs_id, evidence_window)
            {
...
fn embed_with_cache(
    ...
    let mut vs = embedder.embed(&[text]).context("embed (cache miss)")?;
    let v = vs.remove(0);
    cache
...
fn bm25_top_n(facts_db_path: &Path, project: &str, query: &str, n: i32) -> Result<Vec<(i64, f64)>> {
...
fn compute_entity_boost(
    ...
    // Path 1: exact name match.
    {
        ...
        for name in query_entity_names {
            ...
            let entry = per_query_entity_links.entry(name.clone()).or_default();
            for r in rows {
...
/// over-fetch (~60 ids).
fn fetch_meta_for_ids(facts_db_path: &Path, ids: &[i64]) -> Result<HashMap<i64, FactMeta>> {
    if ids.is_empty() {
        return Ok(HashMap::new());
    }
    ...
    for r in rows {
        let (id, salience, temporal, subject, predicate, object) = r.context("read meta row")?;
