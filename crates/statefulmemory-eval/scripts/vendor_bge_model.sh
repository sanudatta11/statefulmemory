#!/usr/bin/env bash
# Vendor BGE-small-en-v1.5 model files into a local directory so statefulmemory-embed
...
#
# Usage:
#   ./scripts/vendor_bge_model.sh                # writes to ~/.statefulmemory-models/bge-small
#   STATEFULMEMORY_BGE_MODEL_DIR=/some/path ./scripts/vendor_bge_model.sh
#
# Then point the eval binary at it:
#   export STATEFULMEMORY_BGE_MODEL_DIR=~/.statefulmemory-models/bge-small
...
DEST="${STATEFULMEMORY_BGE_MODEL_DIR:-$HOME/.statefulmemory-models/bge-small}"
...
if [ "$QUIET" = "1" ]; then
    ...
    echo "  export STATEFULMEMORY_BGE_MODEL_DIR=$DEST"
