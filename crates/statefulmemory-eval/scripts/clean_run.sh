#!/usr/bin/env bash
# Clean rebuild + smoke test for statefulmemory-eval.
# Run from crates/statefulmemory-eval/ (or anywhere; uses script-relative paths).
set -euo pipefail
...
cargo clean -p statefulmemory-eval 2>/dev/null || true
echo "  cleaned: data/projects, reports/, target/release/statefulmemory-eval"
...
echo ""
echo "=== [3/4] Building statefulmemory-eval (release) ==="
cargo build --release -p statefulmemory-eval
