...
//!
//! Used in TCP mode only. The daemon's [`auth::make_interceptor`](../../statefulmemory-daemon/src/auth.rs)
...
/// `statefulmemory team token-create` are 64-char hex and never trigger this path.
pub fn bearer_interceptor(
    ...
    // and break `StatefulMemoryClient::with_interceptor`.
