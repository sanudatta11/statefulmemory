...
const UDS_PLACEHOLDER_URI: &str = "http://statefulmemory.local";
...
pub fn connect_uds(socket: impl AsRef<Path>) -> Result<Channel, ClientError> {
    ...
    let channel =
        endpoint.connect_with_connector_lazy(service_fn(move |_uri: tonic::transport::Uri| {
...
///
/// The CA PEM is the file produced by `statefulmemory team init-ca`. `domain` must
...
pub async fn connect_tcp(addr: &str, ca_pem: &[u8], domain: &str) -> Result<Channel, ClientError> {
...
/// helper in `statefulmemory_daemon::tls`.
