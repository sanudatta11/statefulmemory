//! `memlayer ui` — loopback web dashboard for browsable memory.
//!
...
use memlayer_client::{channel, MemlayerClient};
...
type Client = MemlayerClient<tonic::transport::Channel>;

pub async fn dispatch(project_flag: Option<String>, args: &UiArgs) -> ExitCode {
    if !matches!(args.host.as_str(), "127.0.0.1" | "::1" | "localhost") {
        eprintln!(
            "memlayer: refusing to bind {}. memlayer UI is loopback-only; use a reverse proxy for LAN exposure.",
            args.host,
        );
    ...
    let server = match Server::http(&addr) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("memlayer: ui bind {addr}: {e}");
            return ExitCode::from(exit::GENERAL);
        }
    };
    println!("memlayer UI → http://{addr}     hit Ctrl-C to stop");
...
async fn search(client: Arc<Mutex<Client>>, project: &str, query: &str) -> String {
    ...
    match client
        .lock()
        .unwrap()
        .search_observations(memlayer_proto::SearchObservationsRequest {
...
async fn graph(client: Arc<Mutex<Client>>, project: &str, entity: &str, hops: u8) -> String {
    ...
    let list = match client
        .lock()
        .unwrap()
        .list_entities(memlayer_proto::ListEntitiesRequest {
            project_name: project.into(),
    ...
    match client
        .lock()
        .unwrap()
        .graph_query(memlayer_proto::GraphQueryRequest {
            project_name: project.into(),
            entity_id: id,
            hops: hops as u32,
            edge_types: vec![],
...
async fn stats(client: Arc<Mutex<Client>>, project: &str) -> String {
    match client
        .lock()
        .unwrap()
        .dream_scan(memlayer_proto::DreamScanRequest {
            project_name: project.into(),
        })
        .await
    {
...
async fn decide(client: Arc<Mutex<Client>>, project: &str, question: &str) -> String {
    ...
    match client
        .lock()
        .unwrap()
        .decide(memlayer_proto::DecideRequest {
            project_name: project.into(),
