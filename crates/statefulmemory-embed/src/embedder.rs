...
//!   1. `STATEFULMEMORY_BGE_MODEL_DIR` (explicit vendor path)
//!   2. `~/.statefulmemory-models/bge-small` when already complete
//!   3. `hf-hub` ≥0.4 (joins HuggingFace relative `/api/resolve-cache/…`
//!      redirects; 0.3 failed with `RelativeUrlWithoutBase`)
//!   4. `curl` into `~/.statefulmemory-models/bge-small` if hf-hub still fails
...
fn default_vendor_dir() -> PathBuf {
    dirs_next_home()
        .map(|h| h.join(".statefulmemory-models").join("bge-small"))
        .unwrap_or_else(|| PathBuf::from(".statefulmemory-models/bge-small"))
}
        ...
        if !status.success() {
            ...
            anyhow::bail!(
                "curl failed downloading {name} from {url} (exit {status}); \
                 set STATEFULMEMORY_BGE_MODEL_DIR after running \
                 crates/statefulmemory-eval/scripts/vendor_bge_model.sh"
            );
        }
    }
...
fn resolve_model_paths() -> Result<(PathBuf, PathBuf, PathBuf)> {
    if let Ok(local_dir) = std::env::var("STATEFULMEMORY_BGE_MODEL_DIR") {
        let dir = PathBuf::from(local_dir);
        tracing::info!(?dir, "loading BGE-small from STATEFULMEMORY_BGE_MODEL_DIR");
...
impl BgeSmallEmbedder {
    ...
    /// 1. `STATEFULMEMORY_BGE_MODEL_DIR` — explicit vendor path
    /// 2. `~/.statefulmemory-models/bge-small` if already populated
    /// 3. `hf-hub` (caches under `~/.cache/huggingface/`)
    /// 4. `curl` into `~/.statefulmemory-models/bge-small` when hf-hub fails
    ...
    ///   set `STATEFULMEMORY_BGE_MODEL_DIR` to bypass.
    /// * Disk full while writing to the cache directory.
    pub fn try_new() -> Result<Self> {
        let (config_path, tokenizer_path, weights_path) = resolve_model_paths()?;

        let config_json = std::fs::read_to_string(&config_path).context("read BGE config.json")?;
        let config: Config = serde_json::from_str(&config_json).context("parse BGE config.json")?;
    ...
    fn embed_batch(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        ...
        let input_ids =
            Tensor::from_vec(ids, (batch, seq), &self.device).context("build input_ids tensor")?;
