...
//! Integration tests for `statefulmemory-embed`.
//!
...
use statefulmemory_embed::cache::{fingerprint, hash_key, EmbeddingCache};
...
fn sample_vecs() -> Vec<(String, Vec<f32>)> {
    vec![
        ...
        (
            "rust is fun".to_string(),
            vec![-0.5_f32, 0.0, 1.0, std::f32::consts::PI],
        ),
        ("statefulmemory".to_string(), vec![1e-6_f32, -1e-6, 42.0, -42.0]),
    ]
}
...
fn cache_fingerprint_collision_treated_as_miss() {
    ...
    // an unrelated text. Querying for "a" must detect the mismatch and miss.
    {
        ...
        let payload: Vec<u8> = [9.9_f32, 8.8]
            .iter()
...
//   cargo test --release -p statefulmemory-embed --features online-tests \
...
fn bge_small_embedding_is_deterministic_for_same_text() {
    let emb = statefulmemory_embed::embedder::BgeSmallEmbedder::try_new()
    ...
    assert_eq!(v1.len(), statefulmemory_embed::Embedder::dim(&emb));
...
fn bge_small_distinct_texts_have_distinct_embeddings() {
    let emb = statefulmemory_embed::embedder::BgeSmallEmbedder::try_new().unwrap();
    let v1 = emb.embed(&["the cat sat on the mat"]).unwrap().remove(0);
    let v2 = emb
        .embed(&["airplanes fly through the sky"])
        .unwrap()
        .remove(0);
    ...
    assert!(
        cosine < 0.9,
        "expected distinct embeddings, got cosine {cosine}"
    );
}
