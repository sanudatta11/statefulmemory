...
use crate::logging;
use crate::service::{DaemonState, StatefulMemoryService};
use crate::signals;
use crate::tls;
...
pub async fn run(cfg: Config) -> Result<()> {
    ...
    // Lock + pidfile.
    let lock_file = lifecycle::acquire_lock_and_pid(&paths::lock_path(), &paths::pid_path())?;
    ...
    diskmon::spawn(
        dm.clone(),
        cfg.data_dir.clone(),
        cfg.disk_full_threshold,
        cfg.disk_poll_interval,
    );
    ...
    let global_db = match statefulmemory_storage::GlobalDb::open(&cfg.data_dir) {
    ...
    // Resolve retrieval-pipeline config (statefulmemory config.toml +
    // STATEFULMEMORY_* env vars). The daemon uses this for the embed worker
    // count (and rp-t6 will use it for the extract pool); per-project
    // overrides are re-resolved per task inside the workers.
    let statefulmemory_cfg = statefulmemory_core::config::load_resolved(None);
    ...
    let claude_client: Arc<dyn statefulmemory_extract::claude_cli::ClaudeClient> =
        Arc::new(statefulmemory_extract::claude_cli::ClaudeCliClient::new());
    ...
    let conflict_classifier: std::sync::Arc<
        dyn statefulmemory_storage::conflict_judge::ConflictClassifier,
    > = std::sync::Arc::new(
        crate::conflict_judge::ClaudeConflictClassifier::new(
            claude_client.clone(),
            statefulmemory_cfg.conflict.model,
        )
        .with_timeout(statefulmemory_cfg.conflict.timeout_secs),
    );
    // Re-build registry with the classifier now that claude_client is available.
    let registry_with_judge = Arc::new(
        statefulmemory_storage::registry::ProjectRegistry::new(
            cfg.project_lru_capacity,
            cfg.write_batch_max,
            cfg.write_batch_window,
        )
    ...
    // so flipping the toggle in `~/.statefulmemory/config.toml` takes effect
    // without a daemon restart.
    let extract_pool = {
        let n = statefulmemory_cfg.extract.workers.max(1);
    ...
    let verify_pool = {
        ...
        Some(crate::verify_worker::VerifyWorkerPool::spawn(
            registry.clone(),
            1,
        ))
    };
    ...
    // Signals.
    let mut shutdown_rx =
        signals::install(registry.clone(), dm.clone(), in_flight.clone(), shutdown_tx);
    ...
    // cold machines without STATEFULMEMORY_BGE_MODEL_DIR).
    ...
    let serve_result = if cfg.is_tcp_mode() {
        ...
        spawn_embedder_load(
            state.clone(),
            registry.clone(),
            statefulmemory_cfg.embed.workers.max(1),
        );
    ...
    let _guard = LifecycleGuard {
        ...
        socket_path: if cfg.is_tcp_mode() {
            None
        } else {
            Some(paths::socket_path())
        },
    };
...
fn spawn_embedder_load(state: Arc<DaemonState>, registry: Arc<ProjectRegistry>, workers: usize) {
    tokio::task::spawn_blocking(move || match statefulmemory_embed::BgeSmallEmbedder::try_new() {
        Ok(embedder) => {
...
async fn serve_uds_then_load_embedder(
    ...
    let listener = UnixListener::bind(&sock)
        .map_err(|e| Error::internal(format!("bind UDS {}: {e}", sock.display())))?;
...
fn check_embed_model_on_disk(_registry: &ProjectRegistry) -> Result<()> {
    use statefulmemory_storage::registry::ProjectRegistry as _Reg;
    ...
    for (name, _cfg) in projects {
        ...
        let conn = match statefulmemory_storage::db::open_read(&db_path) {
            Ok(c) => c,
            Err(e) => {
        ...
        pragmas::check_embed_model_compat(&conn, pragmas::STORED_EMBED_MODEL)
            .map_err(|e| Error::FailedPrecondition(format!("project \"{name}\": {e}")))?;
    }
    Ok(())
}
...
async fn serve_tcp(
    ...
    let listen = cfg
        .listen
        .as_ref()
        .ok_or_else(|| Error::FailedPrecondition("STATEFULMEMORY_LISTEN missing".into()))?;
    let addr_str = listen.strip_prefix("tcp://").unwrap_or(listen);
    let addr: std::net::SocketAddr = addr_str
        .parse()
        .map_err(|e| Error::invalid(format!("STATEFULMEMORY_LISTEN parse: {e}")))?;
