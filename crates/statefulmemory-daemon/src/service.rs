//! `StatefulMemoryService`: implements the tonic-generated `StatefulMemory` trait.
//!
//! Each handler:
...
//!  4. Maps the `statefulmemory_core::Error` to a `tonic::Status`.
//!
...
use statefulmemory_proto::{stateful_memory_server::StatefulMemory, Cursor as ProtoCursor, *};
...
pub struct DaemonState {
    ...
    /// Cross-project mirror DB (`~/.statefulmemory/global.sqlite`). The daemon
    ...
    pub query_embedder: std::sync::Arc<
        parking_lot::RwLock<Option<std::sync::Arc<statefulmemory_embed::BgeSmallEmbedder>>>,
    >,
    ...
    pub claude_client: std::sync::Arc<dyn statefulmemory_extract::claude_cli::ClaudeClient>,
...
impl StatefulMemoryService {
    ...
    fn check_writeable(&self) -> Result<()> {
        if self.state.disk_monitor.read_only() {
            return Err(Error::ResourceExhausted(
                "disk full — statefulmemory is in read-only mode".into(),
            ));
        }
        Ok(())
    }
    ...
    pub(crate) fn resolved_search_mode(
        ...
        wire: Option<&str>,
        project_name: &str,
    ) -> statefulmemory_retrieval::hybrid::HybridMode {
        let cfg = statefulmemory_core::config::load_resolved(Some(project_name));
        statefulmemory_retrieval::hybrid::HybridMode::parse_wire_or_default(wire, &cfg.search.mode)
    }

    /// Wire `rerank` if set (forces LLM); otherwise honor config backend.
    /// Returns `None` when rerank is off.
    pub(crate) fn resolved_rerank(
        ...
        if let Some(m) = wire.map(str::trim).filter(|s| !s.is_empty()) {
            return Some(RerankChoice::Llm(m.to_string()));
        }
        let cfg = statefulmemory_core::config::load_resolved(Some(project_name));
        if !cfg.search.rerank {
            return None;
        }
        match cfg.rerank.backend.to_ascii_lowercase().as_str() {
            "off" => None,
            "llm" => Some(RerankChoice::Llm(
                cfg.rerank.model.as_lowercase().to_string(),
            )),
            _ => Some(RerankChoice::Local),
        }
    }
    ...
    pub(crate) fn hybrid_search(
        ...
        self.hybrid_search_tiered(
            ...
            limit,
            project_name,
            crate::query_router::QueryTier::Normal,
        )
    }
    ...
    pub(crate) fn hybrid_search_tiered(
        ...
        let search_q = if tier.use_fact_expand() {
            let expanded = crate::query_expand::expand_query_with_facts(conn, query, 12);
        ...
        let time_range = if tier.use_fact_expand() {
            crate::query_expand::infer_time_range(query, chrono::Utc::now())
        } else {
            None
        };
    ...
    /// Local MiniLM-style CE or LLM rerank. Soft-fails to input order.
    async fn rerank_hits(
        &self,
        choice: &RerankChoice,
        query: &str,
        hits: Vec<Observation>,
        emb_map: &std::collections::HashMap<i64, Vec<f32>>,
    ) -> Vec<Observation> {
        if hits.len() <= 1 {
            return hits;
        }
        match choice {
            RerankChoice::Local => self.local_rerank_hits(query, hits, emb_map),
            RerankChoice::Llm(model_id) => self.llm_rerank_hits(model_id, query, hits).await,
        }
    }

    fn local_rerank_hits(
        &self,
        query: &str,
        hits: Vec<Observation>,
        emb_map: &std::collections::HashMap<i64, Vec<f32>>,
        ...
        let top_n = cfg.rerank.top_n.max(1) as usize;
        let pool: Vec<Observation> = hits.into_iter().take(top_n.max(1)).collect();
        let q_emb = self.embed_query_cached(query);
        let cands: Vec<statefulmemory_retrieval::local_rerank::LocalCandidate<'_>> = pool
            .iter()
            .map(|o| statefulmemory_retrieval::local_rerank::LocalCandidate {
                title: o.title.as_str(),
                content: o.content.as_str(),
                embedding: emb_map.get(&o.id).map(|v| v.as_slice()),
            })
            .collect();
        let (idxs, dur) = statefulmemory_retrieval::local_rerank::local_rerank_indices(
            query,
            &cands,
            q_emb.as_deref(),
            pool.len(),
...
/// Rerank backend selected for one request.
#[derive(Debug, Clone)]
pub(crate) enum RerankChoice {
    Local,
    Llm(String),
}

fn load_obs_embeddings(
    conn: &rusqlite::Connection,
    hits: &[Observation],
) -> std::collections::HashMap<i64, Vec<f32>> {
    let mut out = std::collections::HashMap::new();
    for o in hits {
        if let Ok(Some(v)) = embedding_blob_for(conn, o.id) {
            out.insert(o.id, v);
        }
    }
    out
...
fn embedding_blob_for(conn: &rusqlite::Connection, obs_id: i64) -> Result<Option<Vec<f32>>> {
    // Prefer float32 vec0 blob when present.
    let blob: Option<Vec<u8>> = conn
        .query_row(
            "SELECT embedding FROM observations_vec WHERE rowid = ?1",
            [obs_id],
            |r| r.get(0),
        )
        .optional()
        .map_err(|e| Error::internal(format!("load embedding: {e}")))?;
    if let Some(b) = blob {
        if b.len() >= 4 && b.len() % 4 == 0 {
...
///
/// The write thread sends `Result<T, statefulmemory_core::Error>` over a
...
fn filter_context_observations(
    observations: &mut Vec<statefulmemory_proto::Observation>,
    project_name: &str,
    include_stale: bool,
) {
    let cfg = statefulmemory_core::config::load_resolved(Some(project_name));
...
fn graph_expansion(
    ...
    for name in &seed_names {
        ...
        let matches = statefulmemory_storage::graph::entity_lookup(conn, &norm, 20);
        ...
        // HippoRAG-2-lite synonymy: also take near-prefix neighbors as soft seeds.
        for e in matches.iter().take(3) {
            if e.norm_name != norm && !seed_ids.contains(&e.id) {
                seed_ids.push(e.id);
            }
        }
    ...
    seed_ids.truncate(usize::from(cfg.max_query_entities).max(1) * 2);
    ...
    let use_ppr = cfg.ranker.eq_ignore_ascii_case("ppr");
    ...
    let obs_ids = statefulmemory_storage::graph::observations_for_entities(conn, &entity_ids, 64);
    if obs_ids.is_empty() {
        return out;
    }

    // HippoRAG-style PPR: score entities, then aggregate onto observations.
    let ppr_scores: Option<std::collections::HashMap<i64, f64>> = if use_ppr {
        match statefulmemory_storage::graph::edges_for_entities(conn, &entity_ids, &edge_types, 512)
        {
            Ok(edges) => {
                let weighted: Vec<(i64, i64, f64)> = edges
                    .iter()
                    .map(|e| (e.from_id, e.to_id, e.weight.max(0.01)))
                    .flat_map(|(a, b, w)| [(a, b, w), (b, a, w)])
                    .collect();
                let mut specificity = std::collections::HashMap::new();
                for &id in &entity_ids {
                    let n: i64 = conn
                        ...
                        .unwrap_or(1);
                    specificity.insert(id, 1.0 / (n.max(1) as f64));
                }
                Some(statefulmemory_retrieval::ppr::ppr_entity_scores(
                    &entity_ids,
                    &weighted,
                    &seed_ids,
                    cfg.ppr_damping,
    ...
    // With PPR, observation score = sum of PPR(entity) × boost for mentioned
    // entities in the frontier.
    let mut scored: Vec<(i64, f64)> = Vec::new();
    let mut budget_used = 0usize;
    for &obs_id in &obs_ids {
        let obskey = statefulmemory_storage::write::ObservationKey::Id(obs_id);
        ...
        let score = if let Some(ref ppr) = ppr_scores {
            ...
            let hits_for_obs: Vec<GraphHit> = cand
                ...
                .map(|(entity_id, _, hop)| GraphHit {
                    ...
                    hop: *hop,
                    edge_weight: 1.0,
                    relation: statefulmemory_core::config::EdgeRelation::Mentions,
                })
                .collect();
            if hits_for_obs.is_empty() {
                continue;
            }
            graph_score(&hits_for_obs, cfg.boost)
                .first()
                .map(|(_, s)| *s)
                .unwrap_or(0.0)
        };
        ...
        // HippoRAG-2-lite recognition filter: drop graph-only lifts with zero
        // lexical overlap to the query (heuristic stand-in for LLM recognition).
        if !token_overlap_nonzero(query, &o.title, &o.content) {
            continue;
        }
    ...
    for (obs_id, _score) in scored {
        let obskey = statefulmemory_storage::write::ObservationKey::Id(obs_id);
        if let Ok(o) = statefulmemory_storage::read::get(conn, &obskey) {
...
fn token_overlap_nonzero(query: &str, title: &str, content: &str) -> bool {
    ...
    let blob = format!("{title} {content}").to_ascii_lowercase();
    q.iter().any(|t| blob.contains(t.as_str()))
}
...
async fn stamp_observation_anchors(
    ...
    use statefulmemory_storage::anchor::{digest_slice, Anchor, VerifyState};
    ...
    let Some(repo) = repo.filter(|p| git::is_repo(p)) else {
        ...
        map(project.write.send(WriteRequest::Custom {
            f: Box::new(move |conn| {
                statefulmemory_storage::anchor::insert_anchors(conn, observation_id, &anchors)?;
                statefulmemory_storage::anchor::set_verify_state(
                    conn,
                    observation_id,
    ...
    map(project.write.send(WriteRequest::Custom {
        f: Box::new(move |conn| {
            ...
            if let Some(first) = anchors.first() {
                ...
                .map_err(|e| statefulmemory_core::Error::internal(format!("code_anchor: {e}")))?;
            }
            Ok(())
        }),
        reply: tx,
...
fn attach_supersedes(
    conn: &rusqlite::Connection,
    observations: &mut [statefulmemory_proto::Observation],
) {
...
fn fact_to_proto(f: facts_q::Fact) -> statefulmemory_proto::Fact {
    statefulmemory_proto::Fact {
        id: f.id,
        obs_id: f.obs_id,
        subject: f.subject,
...
fn session_to_proto(s: Session) -> statefulmemory_proto::Session {
    statefulmemory_proto::Session {
        id: s.id,
...
fn prompt_to_proto(p: Prompt) -> statefulmemory_proto::Prompt {
    statefulmemory_proto::Prompt {
        id: p.id,
        sync_id: p.sync_id,
...
// ---------------------------------------------------------------------------
// StatefulMemory trait implementation
// ---------------------------------------------------------------------------

#[tonic::async_trait]
impl StatefulMemory for StatefulMemoryService {
    async fn list_entities(
        ...
        if !r.kind_filter.is_empty() {
            if statefulmemory_core::config::EntityKind::parse(&r.kind_filter).is_none() {
        ...
        Ok(Response::new(statefulmemory_proto::ListEntitiesResponse {
    ...
    async fn graph_query(
        ...
        let degree_cap = statefulmemory_core::config::load_resolved(Some(&r.project_name))
            .graph
            .degree_cap;
        ...
        Ok(Response::new(statefulmemory_proto::GraphQueryResponse {
            entities: entities.into_iter().map(entity_to_proto).collect(),
    ...
    async fn save_observation(
        ...
        // BM25-ranked view across every statefulmemory-tracked repo. Failure here
        ...
        if statefulmemory_core::config::load_resolved(Some(&r.project_name))
            .graph
            .enabled
        {
        ...
        let superseded: Vec<statefulmemory_proto::Observation> = obs
            .superseded_ids
            .iter()
            .map(|&sid| statefulmemory_proto::Observation {
                id: sid,
                ..Default::default()
            })
            .collect();
    ...
    async fn search_observations(
        ...
        if r.all_projects {
            ...
            if mode == statefulmemory_retrieval::hybrid::HybridMode::Hybrid {
                ...
                if let Some(embedder) = self.state.query_embedder.read().as_ref() {
                    ...
                    if let Some(q_vec) = q_vec {
                        ...
                        let project_paths: Vec<(String, std::path::PathBuf)> = projects
                            .into_iter()
                            .map(|(name, _)| {
                                let path = statefulmemory_core::paths::project_db_path(&name);
                                (name, path)
                            })
                            .collect();
                        ...
                        let fused_keys = statefulmemory_retrieval::rrf::rrf_fuse_keyed(
                        ...
                        let observations: Vec<statefulmemory_proto::Observation> = fused_keys
                            .into_iter()
                            .take(limit as usize)
            ...
            if let Some(global) = &self.state.global_db {
                ...
                let observations: Vec<statefulmemory_proto::Observation> = hits
                    .into_iter()
                    .map(|h| statefulmemory_proto::Observation {
                        id: h.source_id,
                        sync_id: String::new(),
                        session_id: String::new(),
                        r#type: h.r#type,
        ...
        if let Some(ids) = self.state.search_cache.get(&cache_key) {
            let mut hits = Vec::new();
            for id in ids {
                if let Ok(o) = read_q::get(&conn, &ObservationKey::Id(id)) {
                    hits.push(o);
                }
            }
        ...
        let hits = match self.resolved_rerank(r.rerank.as_deref(), &r.project_name) {
            Some(choice) if tier.use_local_rerank() || matches!(choice, RerankChoice::Llm(_)) => {
                let emb_map = load_obs_embeddings(&conn, &hits);
                self.rerank_hits(&choice, &r.query, hits, &emb_map).await
            }
            _ => hits,
        };
        let ids: Vec<i64> = hits.iter().map(|o| o.id).collect();
        self.state.search_cache.put(cache_key, ids);
        let hits = crate::token_budget::apply_max_per_type(hits, cfg.search.max_per_type);
        let max_tokens = r.max_tokens.unwrap_or(0).max(0) as u32;
        let (hits, tokens_used) = crate::token_budget::pack_by_token_budget(hits, max_tokens);
    ...
    async fn context(
        ...
        let mut allow_graph_query: Option<&str> = None;
        let recents = match query_ref {
            Some(q) => {
                ...
                let hits = match self.resolved_rerank(r.rerank.as_deref(), &r.project_name) {
                    Some(choice)
                        if tier.use_local_rerank() || matches!(choice, RerankChoice::Llm(_)) =>
                    {
                        let emb_map = load_obs_embeddings(&conn, &hits);
                        self.rerank_hits(&choice, q, hits, &emb_map).await
                    }
                    _ => hits,
                };
    ...
    async fn get_observation_history(
        ...
        let proto_entries = entries
            .into_iter()
            .map(|e| statefulmemory_proto::ObservationHistoryEntry {
    ...
    async fn reextract_observations(
        ...
        if !cfg.extract.enabled {
            return Err(Status::failed_precondition(
                "extract is disabled for this project; run: statefulmemory config set extract.enabled true",
            ));
        }
    ...
    async fn reindex_observations(
        ...
        if r.force {
            ...
            map(project
                .write
                .send(statefulmemory_storage::write::WriteRequest::Custom {
                    f: Box::new(|conn| {
    ...
    async fn verify_anchors(
        ...
        let head = match statefulmemory_core::git::head_sha(&repo) {
            Ok(h) => h,
            Err(_) => {
                return Ok(Response::new(VerifyAnchorsResponse::default()));
            }
        ...
        let pairs = match r.observation_id {
            ...
            None => {
                let listed = map(statefulmemory_storage::anchor::list_anchored(&conn, 10_000))?;
                listed
                    .into_iter()
        ...
        for v in &verdicts {
            ...
            match v.state {
                statefulmemory_storage::VerifyState::Verified => resp.verified += 1,
                statefulmemory_storage::VerifyState::Stale => resp.stale += 1,
                statefulmemory_storage::VerifyState::Invalidated => resp.invalidated += 1,
                statefulmemory_storage::VerifyState::Unprovable => resp.unprovable += 1,
                statefulmemory_storage::VerifyState::Unanchored => resp.unanchored += 1,
            }
    ...
    async fn current_project(
        ...
        let normalized = map(statefulmemory_core::project::normalize(basename))?;
    ...
    async fn merge_projects(
        ...
        let from_norm = map(statefulmemory_core::project::normalize(&r.from))?;
        let to_norm = map(statefulmemory_core::project::normalize(&r.to))?;
        let from_path = statefulmemory_core::paths::project_db_path(&from_norm);
        let to_path = statefulmemory_core::paths::project_db_path(&to_norm);
        if !from_path.exists() {
        ...
        let outcome: std::sync::Arc<
            std::sync::Mutex<Option<statefulmemory_storage::projects_admin::MergeOutcome>>,
        ...
        map(project
            ...
            .send(statefulmemory_storage::write::WriteRequest::Custom {
                f: Box::new(move |conn| {
                    let merged = statefulmemory_storage::projects_admin::merge_into_target_conn(
                        conn,
                        &from_for_closure,
                    )?;
        ...
        map(reply_rx.await.map_err(|_| {
            statefulmemory_core::error::Error::Unavailable(
    ...
    async fn delete_project(
        ...
        if inner.hard {
            ...
            let db = statefulmemory_core::paths::project_db_path(&normalized);
    ...
    async fn dream_scan(
        ...
        let props = map(statefulmemory_storage::dream::dream_scan(&conn))?;
        let proposals = props
            .iter()
            .map(|p| {
                let (keep_title, drop_title) =
                    (obs_title(&conn, p.keep_id), obs_title(&conn, p.drop_id));
                statefulmemory_proto::DreamProposalProto {
                    kind: p.kind.clone(),
                    keep_id: p.keep_id,
                    drop_id: p.drop_id,
            ...
            .collect();
        Ok(Response::new(statefulmemory_proto::DreamScanResponse {
            proposals,
...
mod tests {
    ...
    fn graph_expansion_returns_co_mentioning_observations() {
        statefulmemory_storage::pragmas::ensure_sqlite_vec_extension();
