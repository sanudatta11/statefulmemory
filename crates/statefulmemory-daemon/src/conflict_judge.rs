...
//! proxy-strip pattern as `statefulmemory-extract`). It runs synchronously from
...
use statefulmemory_extract::claude_cli::ClaudeClient;
use statefulmemory_storage::conflict_judge::{ConflictClassifier, ConflictVerdict};
        ...
        let raw = with_rt(|rt| {
            rt.block_on(async move {
                ...
                tokio::time::timeout(timeout, fut)
                    .await
                    .map_err(|_| {
                        anyhow::anyhow!("conflict judge timed out after {}s", timeout.as_secs())
                    })?
...
mod tests {
    ...
    fn parse_conflicts_with() {
        assert_eq!(
            parse_verdict("CONFLICTS_WITH").unwrap(),
            ConflictVerdict::ConflictsWith
        );
    }
    ...
    fn parse_compatible() {
        assert_eq!(
            parse_verdict("COMPATIBLE").unwrap(),
            ConflictVerdict::Compatible
        );
    }
    ...
    fn parse_not_conflict() {
        assert_eq!(
            parse_verdict("NOT_CONFLICT").unwrap(),
            ConflictVerdict::NotConflict
        );
        assert_eq!(
            parse_verdict("not_conflict extra text").unwrap(),
            ConflictVerdict::NotConflict
        );
    }
