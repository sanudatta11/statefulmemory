...
use statefulmemory_proto::{DecideConflict, DecideEvidence, DecideRequest, DecideResponse};
use statefulmemory_retrieval::hybrid::HybridMode;
use statefulmemory_storage::write::{SaveObservationInput, WriteRequest};
use statefulmemory_storage::{read as read_q, Observation};

use crate::error_map::map;
use crate::resolve_worker::{self, extract_json_object};
use crate::service::StatefulMemoryService;
...
pub async fn handle(svc: &StatefulMemoryService, req: DecideRequest) -> Result<DecideResponse, Status> {
    ...
    if parsed.should_record && parsed.confidence >= 0.7 {
        ...
        map(project.write.send(WriteRequest::SaveObservation {
            input: SaveObservationInput {
                ...
                tool_name: None,
                scope: "project".into(),
                created_by: Some("statefulmemory-decide".into()),
...
fn load_conflicts(
    ...
    let mut out = Vec::new();
    for id in ids {
        let rels = map(statefulmemory_storage::get_relations_for_observation(&conn, *id))?;
        for r in rels {
...
fn pair_resolved(conn: &rusqlite::Connection, a: i64, b: i64) -> bool {
    let Ok(rels_a) = statefulmemory_storage::get_relations_for_observation(conn, a) else {
        return false;
    };
    let Ok(rels_b) = statefulmemory_storage::get_relations_for_observation(conn, b) else {
        return false;
    };
    rels_a
        .iter()
...
pub(crate) fn parse_decide_json(raw: &str) -> anyhow::Result<DecideParsed> {
    let slice = extract_json_object(raw)
        .ok_or_else(|| anyhow::anyhow!("no JSON object in decide output"))?;
