...
//!
//! Log file: `~/.statefulmemory/daemon.log` (JSON, one record per line).
//! Level:    `STATEFULMEMORY_LOG` env var (default: `debug`).
//!           Examples: `STATEFULMEMORY_LOG=trace`, `STATEFULMEMORY_LOG=info`.
//!
//! Read the log:
//!   statefulmemory logs --lines 100
//!   tail -f ~/.statefulmemory/daemon.log | jq .
//!   cat ~/.statefulmemory/daemon.log | jq 'select(.level=="ERROR")'
...
/// restart. Override with `STATEFULMEMORY_LOG=info` (or trace/warn/error).
pub fn init(
    log_path: &Path,
    env_filter: &str,
    ...
    // Callers pass the value from Config.log_level (which reads STATEFULMEMORY_LOG).
    let effective = if env_filter.trim().is_empty() {
        "debug"
    } else {
        env_filter
    };
