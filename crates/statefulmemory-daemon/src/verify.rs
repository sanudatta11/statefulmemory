...
use statefulmemory_storage::anchor::{digest_slice, locate_symbol, Anchor, VerifyState};
...
pub fn verify_anchors(repo: &Path, head: &str, anchors: &[(i64, Anchor)]) -> Vec<AnchorVerdict> {
...
fn verify_one(
    ...
    let changed = changed_cache
        ...
        .or_insert_with(|| git::changed_paths(repo, commit, head).unwrap_or_default());
