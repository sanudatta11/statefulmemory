...
use statefulmemory_proto::{ExportMemRequest, ExportMemResponse};
use statefulmemory_sync::mem_archive::{encode, EncodeParams};
use statefulmemory_sync::snapshot;
...
#[allow(clippy::result_large_err)]
pub(crate) fn map_sync(e: statefulmemory_sync::SyncError) -> Status {
    use statefulmemory_sync::SyncError;
    match e {
        ...
        SyncError::NotArchive => Status::invalid_argument("not a statefulmemory archive"),
        SyncError::Truncated => Status::invalid_argument("statefulmemory archive truncated"),
        SyncError::ChecksumMismatch => {
            Status::invalid_argument("statefulmemory archive checksum mismatch")
        }
        SyncError::UnsupportedVersion(v) => {
            Status::invalid_argument(format!("unsupported statefulmemory archive version {v}"))
        }
