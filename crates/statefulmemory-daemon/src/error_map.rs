//! Convert `statefulmemory_core::Error` → `tonic::Status`.
//!
...
//! `statefulmemory-error-info-bin` metadata header. That has been removed
...
use statefulmemory_core::error::{Error, ErrorKind};
use tonic::Status;
...
/// Helper: short-circuit Result<T, statefulmemory::Error> → Result<T, tonic::Status>.
#[allow(clippy::result_large_err)]
pub fn map<T>(r: statefulmemory_core::Result<T>) -> Result<T, Status> {
    r.map_err(to_status)
}
...
mod tests {
    ...
    fn every_error_kind_builds_status_without_panic() {
        let cases: &[(Error, Code)] = &[
            ...
            (
                Error::Unavailable("backpressured".to_string()),
                Code::Unavailable,
            ),
            (
