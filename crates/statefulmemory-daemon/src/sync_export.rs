...
use statefulmemory_proto::{SyncExportRequest, SyncExportResponse};
use statefulmemory_storage::{sync_state, ProjectState};
use statefulmemory_sync::{chunk, manifest};
...
pub async fn run(
    ...
    let jsonl = map(chunk::build_jsonl(&observations, &sessions, &prompts)
        .map_err(|e| statefulmemory_core::error::Error::internal(e.to_string())))?;
