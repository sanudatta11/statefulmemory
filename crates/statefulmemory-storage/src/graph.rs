...
pub fn get_entity_by_id(conn: &Connection, id: i64) -> Result<statefulmemory_core::Entity> {
    conn.query_row(
...
fn entity_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<statefulmemory_core::Entity> {
    Ok(statefulmemory_core::Entity {
        id: row.get(0)?,
        kind: statefulmemory_core::config::EntityKind::parse(&row.get::<_, String>(1)?)
            .unwrap_or(statefulmemory_core::config::EntityKind::Concept),
        name: row.get(2)?,
        norm_name: row.get(3)?,
    })
}
