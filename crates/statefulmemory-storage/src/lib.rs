...
//! Per-project SQLite+FTS5 storage for statefulmemory.
//!
//! Each project gets its own database file at `~/.statefulmemory/projects/<id>.db`.
...
pub fn run_migrations(conn: &mut rusqlite::Connection) -> statefulmemory_core::Result<()> {
    migrations::migrations::runner()
        .run(conn)
        .map_err(|e| statefulmemory_core::Error::internal(format!("migration failed: {e}")))?;
    Ok(())
}
