...
fn search_dense_quantized(
    ...
    use statefulmemory_embed::quantize::int8_to_f32;
    ...
    for r in rows {
        let (obs_id, blob, scale) =
            r.map_err(|e| Error::internal(format!("search_dense_quantized row: {e}")))?;
        if blob.len() != 384 {
...
pub fn supersedes_ids_for(
    ...
    for r in rows {
        let (parent, child) =
            r.map_err(|e| Error::internal(format!("supersedes_ids_for row: {e}")))?;
...
pub fn search_by_anchor(conn: &Connection, anchor: &str, limit: i32) -> Result<Vec<Observation>> {
    let limit = limit.clamp(1, MAX_LIMIT);
...
mod tests {
    ...
    fn search_dense_quantized_returns_closest_row() {
        ...
        assert_eq!(
            hits[0].id, ids[0],
            "exact-match row should rank first (cosine sim ≈ 1.0)"
        );
    }
    ...
    fn search_dense_quantized_excludes_soft_deleted() {
        ...
        assert_eq!(
            hits.len(),
            1,
            "soft-deleted row must not surface in quantized search"
        );
        assert_eq!(hits[0].id, ids[0]);
    }
}
