...
//!
//! Lives at `~/.statefulmemory/global.sqlite`. The daemon mirrors every successful
...
/// Owns the connection to `~/.statefulmemory/global.sqlite`. One instance per
...
mod tests {
    ...
    fn upsert_replaces_on_duplicate_project_source_id() {
        ...
        db.upsert_observation("repo-a", &obs(1, "first", "v1"))
            .unwrap();
        db.upsert_observation("repo-a", &obs(1, "first", "v2"))
            .unwrap();
        let count: i64 = db
            .conn
            .query_row(
    ...
    fn search_returns_bm25_ranked_hits_across_projects() {
        ...
        db.upsert_observation(
            "repo-a",
            &obs(1, "use pgx not gorm", "team prefers raw sql"),
        )
        .unwrap();
    ...
    fn manifest_tracks_per_project_count() {
        let dir = TempDir::new().unwrap();
        let mut db = GlobalDb::open(dir.path()).unwrap();
        db.upsert_observation("repo-a", &obs(1, "t1", "c1"))
            .unwrap();
        db.upsert_observation("repo-a", &obs(2, "t2", "c2"))
            .unwrap();
