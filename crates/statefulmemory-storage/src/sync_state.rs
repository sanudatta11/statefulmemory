...
/// by `statefulmemory-sync::chunk::build_jsonl`. Only the dedicated partial indexes are
...
/// `<repo>/.statefulmemory/manifest.json` is the source of truth. `SyncStatus`
...
pub fn last_imported_at(conn: &Connection) -> Result<Option<String>> {
    let val: Option<String> = conn
        .query_row("SELECT MAX(imported_at) FROM sync_chunks", [], |row| {
            row.get(0)
        })
        .optional()
