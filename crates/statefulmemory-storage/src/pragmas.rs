...
/// `BAAI/bge-small-en-v1.5` model id used by `statefulmemory-embed`, sans HF org
...
/// many threads race to invoke it. Pattern copied from `statefulmemory-eval/src/vec_index.rs`.
...
/// pointing at `statefulmemory reindex`. An empty meta table (no embeddings yet)
    ...
    match stored {
        ...
        Some(s) => Err(Error::FailedPrecondition(format!(
            "embed model mismatch: stored \"{s}\", configured \"{configured}\"; \
             run `statefulmemory reindex` to clear and re-embed"
        ))),
    }
}
...
mod tests {
    ...
    fn vec0_virtual_table_creatable() {
        ensure_sqlite_vec_extension();
        let conn = Connection::open_in_memory().unwrap();
        conn.execute("CREATE VIRTUAL TABLE vt USING vec0(embedding float[8])", [])
    ...
    fn embed_model_compat_fails_on_mismatch() {
        ...
        let msg = format!("{err}");
        assert!(msg.contains("statefulmemory reindex"), "msg: {msg}");
