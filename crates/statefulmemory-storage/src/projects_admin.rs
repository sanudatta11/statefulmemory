...
/// instead of reading `STATEFULMEMORY_DATA_DIR`. The public API delegates here; tests
...
pub fn list_projects_with_counts_at(projects_dir: &Path) -> Result<Vec<ProjectCounts>> {
        ...
        .map_err(|e| Error::internal(format!("read_dir {}: {e}", projects_dir.display())))?
    {
        ...
        if statefulmemory_core::project::validate(&normalized).is_err() {
            continue;
        }
...
/// without touching `STATEFULMEMORY_DATA_DIR` or the project registry.
...
mod tests {
    ...
    fn make_project(data_dir: &Path, display_name: &str) -> PathBuf {
        let normalized = statefulmemory_core::project::normalize(display_name).unwrap();
    ...
    /// `STATEFULMEMORY_DATA_DIR` (which is a process-global the rest of the test
    ...
    fn consolidate_uses_jaro_winkler() {
        ...
        // deterministic regardless of STATEFULMEMORY_DATA_DIR / parallel test order.
        let projects = vec![
            pc("statefulmemory", 3),
            pc("statefulmemory-cli", 0),
            pc("totally-other", 1),
        ];
        let pairs = consolidate_pairs(&projects, 0.85);

        // statefulmemory ~ statefulmemory-cli must be flagged. statefulmemory ~ totally-other
        // is well below 0.85 and must not appear.
        assert!(
            pairs.iter().any(|c| {
                let names = (c.from.as_str(), c.to.as_str());
                names == ("statefulmemory-cli", "statefulmemory") || names == ("statefulmemory", "statefulmemory-cli")
            }),
            "expected statefulmemory/statefulmemory-cli pair, got {pairs:?}"
        );
        assert!(
            !pairs
                .iter()
