//! Error type for the statefulmemory MCP server and its mapping to MCP wire errors.
//!
...
pub enum McpError {
    ...
    #[error("statefulmemory daemon is not running")]
    DaemonNotRunning,
...
impl McpError {
    ...
    pub fn remedy(&self) -> Option<&'static str> {
        match self {
            McpError::DaemonNotRunning => Some("start the daemon with `statefulmemory daemon start`"),
            McpError::SocketMissing { .. } => {
                Some("start the daemon with `statefulmemory daemon start` to create the socket")
            }
