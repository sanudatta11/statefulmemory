...
impl MemoryServer {
    ...
    async fn memory_facts(
        ...
        let req = statefulmemory_proto::GetFactsRequest {
            project_name: project,
    ...
    async fn memory_recent(
        ...
        let project = resolve_project(&self.base_project, args.project.as_deref())?;
        let req = statefulmemory_proto::RecentObservationsRequest {
            project_name: project,
            limit: clamp_limit(args.limit, 10),
            scope: args.scope,
        };
    ...
    async fn memory_decide(
        ...
        let req = statefulmemory_proto::DecideRequest {
            project_name: project,
    ...
    async fn memory_graph_query(
        ...
        let project = resolve_project(&self.base_project, args.project.as_deref())?;
        let norm = statefulmemory_core::config::normalize_entity_name(&args.entity);
        if norm.is_empty() {
            return Err(McpError::BadArgs {
        ...
        let relation_filter = match &args.relation_filter {
            None => String::new(),
            Some(r) => match statefulmemory_core::config::EdgeRelation::parse(r) {
                Some(_) => r.clone(),
                None => {
                    return Err(McpError::BadArgs {
        ...
        let list = self
            ...
            .call(|mut c| {
                ...
                let norm = norm.clone();
                async move {
                    c.list_entities(statefulmemory_proto::ListEntitiesRequest {
                        project_name: project,
        ...
        let req = statefulmemory_proto::GraphQueryRequest {
            project_name: project,
            entity_id: seed.id,
    ...
    async fn memory_health(
        ...
        let status = match self
            ...
            .call(|mut c| async move {
                c.daemon_status(statefulmemory_proto::DaemonStatusRequest {})
                    .await
                    .map(|r| r.into_inner())
            })
            .await
        ...
        let doctor = self
            ...
            .call(|mut c| {
                let project = project.clone();
                async move {
                    c.doctor(statefulmemory_proto::DoctorRequest {
...
fn pick_entity<'a>(
    entities: &'a [statefulmemory_proto::GraphEntity],
    norm: &str,
) -> Option<&'a statefulmemory_proto::GraphEntity> {
...
impl ServerHandler for MemoryServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            ...
            .with_instructions(
                "statefulmemory local memory tools. Prefer memory_search / memory_recent / \
...
"#,
        ...
        assert!(
            v2["result"]["serverInfo"]["name"] == "statefulmemory-mcp"
