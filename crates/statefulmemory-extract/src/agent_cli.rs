...
//!
//! statefulmemory is not tied to one vendor. Extract, conflict, rerank, resolve,
//! and Decide call whichever agent binary is on PATH (or
//! `STATEFULMEMORY_LLM_BIN` / `STATEFULMEMORY_LLM_PROVIDER`).
//!
...
fn specs() -> &'static [ProviderSpec] {
    ...
    // Override anytime with STATEFULMEMORY_LLM_BIN / STATEFULMEMORY_LLM_PROVIDER.
    &[
        (
            "opencode",
            &["opencode"],
...
///
/// Order: `STATEFULMEMORY_LLM_BIN` → `STATEFULMEMORY_LLM_PROVIDER` → config-dir match
/// with a binary on PATH → first known binary on PATH.
pub fn detect_provider() -> Option<Provider> {
    if let Ok(bin) = std::env::var("STATEFULMEMORY_LLM_BIN") {
        let bin = bin.trim();
        if !bin.is_empty() {
            let hinted = std::env::var("STATEFULMEMORY_LLM_PROVIDER")
                .ok()
    ...
    if let Ok(p) = std::env::var("STATEFULMEMORY_LLM_PROVIDER") {
...
///
/// Explicit `STATEFULMEMORY_LLM_MODEL` always wins. Roles (`fast`/`haiku`/…) use
/// the host's current model env if set, otherwise omit `--model`.
pub fn effective_requested(caller: &str) -> String {
    if let Some(v) = nonempty_env(&["STATEFULMEMORY_LLM_MODEL", "STATEFULMEMORY_CLAUDE_MODEL"]) {
        return v;
    }
...
mod tests {
    ...
    fn opencode_maps_zen_catalog_and_nicknames() {
        ...
        assert_eq!(
            map_model(&p, "qwen").as_deref(),
            Some("opencode/qwen3.7-plus")
        );
        assert_eq!(map_model(&p, "haiku"), None);
        assert_eq!(
