...
/// peers. Override with `STATEFULMEMORY_LLM_BIN` / `STATEFULMEMORY_LLM_PROVIDER`.
/// `STATEFULMEMORY_LLM_MODEL` (or legacy `STATEFULMEMORY_CLAUDE_MODEL`) forces a model id.
        ...
        let Some(provider) = agent_cli::detect_provider() else {
            bail!(
                "no agent CLI found for LLM calls (looked for {}). \
                 Install your agent's CLI or set STATEFULMEMORY_LLM_BIN.",
