...
const STOPWORDS: &[&str] = &[
    ...
    "has", "had", "this", "that", "these", "those", "it", "its", "they", "them", "i", "me", "my",
    "you", "your", "we", "us", "our", "would", "could", "should", "will", "shall", "may", "might",
    "can",
];
    ...
    async fn extract(&self, fact: &Fact) -> Result<Vec<String>> {
        ...
        match self.client.ask(&prompt, HAIKU_MODEL).await {
            Ok(raw) => match Self::parse_entities(&raw) {
                ...
                _ => {
                    tracing::warn!(
                        target: "statefulmemory_extract::entities",
