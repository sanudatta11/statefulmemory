...
//!
//! API mirrors `statefulmemory_embed::cache::EmbeddingCache` (sha256 key,
    ...
    pub fn put_many(&self, items: &[(String, Vec<Fact>)]) -> Result<()> {
        ...
        let tx = conn.transaction().context("begin extraction cache tx")?;
        {
            ...
            for (window, facts) in items {
                ...
                let payload = serde_json::to_string(facts).context("serialize facts for cache")?;
...
mod tests {
    ...
    fn put_then_get_roundtrips_facts() {
        ...
        c.put_many(&[("window-A".to_string(), vec![f.clone()])])
            .unwrap();
