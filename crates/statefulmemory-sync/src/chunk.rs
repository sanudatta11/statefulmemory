...
use statefulmemory_storage::models::{Observation, Prompt, Session};
...
pub fn build_jsonl(obs: &[Observation], sess: &[Session], prompts: &[Prompt]) -> Result<Vec<u8>> {
...
/// Read `STATEFULMEMORY_ZSTD_LEVEL` env var; default 19.
pub fn zstd_level() -> i32 {
    std::env::var("STATEFULMEMORY_ZSTD_LEVEL")
        .ok()
...
pub fn compress_sync(data: &[u8]) -> Result<Vec<u8>> {
    ...
    zstd::encode_all(data, level).map_err(|e| SyncError::Compress(e.to_string()))
}
...
pub fn decompress_sync(data: &[u8]) -> Result<Vec<u8>> {
    zstd::decode_all(data).map_err(|e| SyncError::Decompress(e.to_string()))
}
...
mod tests {
    ...
    async fn write_chunk_atomic_is_idempotent() {
        ...
        write_chunk_atomic(&chunks_dir, id, b"v1".to_vec())
            .await
            .unwrap();
        // Second call with different data should NOT overwrite (immutability).
        write_chunk_atomic(&chunks_dir, id, b"v2".to_vec())
            .await
            .unwrap();
        let path = chunks_dir.join(format!("{id}.jsonl.zst"));
