...
//! `<repo>/.statefulmemory/manifest.json` — append-only chunk registry.
//!
...
/// `<repo>/.statefulmemory/` directory for a given repo root.
pub fn statefulmemory_dir(repo_root: &Path) -> PathBuf {
    repo_root.join(".statefulmemory")
}

/// `<repo>/.statefulmemory/chunks/` directory.
pub fn chunks_dir(repo_root: &Path) -> PathBuf {
    statefulmemory_dir(repo_root).join("chunks")
}

/// `<repo>/.statefulmemory/manifest.json` path.
pub fn manifest_path(repo_root: &Path) -> PathBuf {
    statefulmemory_dir(repo_root).join("manifest.json")
}
...
pub async fn append_atomic(repo_root: &Path, entry: ManifestEntry) -> Result<()> {
    let dir = statefulmemory_dir(repo_root);
...
mod tests {
    ...
    async fn append_atomic_accumulates_entries() {
        ...
        for i in 0u8..3 {
            ...
            append_atomic(dir.path(), new_entry(id, 1, 0, 0))
                .await
                .unwrap();
        }
        let m = read(dir.path()).await.unwrap();
    ...
    async fn path_helpers_compose() {
        ...
        assert_eq!(statefulmemory_dir(root), Path::new("/tmp/repo/.statefulmemory"));
        assert_eq!(chunks_dir(root), Path::new("/tmp/repo/.statefulmemory/chunks"));
        assert_eq!(
            manifest_path(root),
            Path::new("/tmp/repo/.statefulmemory/manifest.json")
        );
    }
}
