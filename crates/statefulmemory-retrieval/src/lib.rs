//! `statefulmemory-retrieval` — shared retrieval primitives.
//!
//! This crate exists so the daemon (`/catalyst-spec retrieval-promotion` rp-t8,
//! rp-t9) and the eval harness (`statefulmemory-eval`) can share one implementation
//! of:
//!
...
//!   version).
//!
//! Pure file-move from `statefulmemory-eval` plus thin shared types. `statefulmemory-eval`
