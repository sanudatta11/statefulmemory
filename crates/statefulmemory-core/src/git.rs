...
/// Repo-relative paths of tracked files (`git ls-files -z`). Empty when not a repo.
pub fn tracked_files(dir: &Path) -> Result<Vec<String>> {
    if !is_repo(dir) {
        return Ok(Vec::new());
    }
    let out = run_git(dir, &["ls-files", "-z"])?;
    Ok(out
        .split('\0')
        .map(|l| l.trim().replace('\\', "/"))
        .filter(|l| !l.is_empty())
...
mod tests {
    ...
    fn head_and_changed_paths_round_trip() {
        ...
        assert!(tracked.iter().any(|p| p == "a.txt"));
    }

    #[test]
    fn tracked_files_empty_outside_repo() {
        let dir = TempDir::new().unwrap();
