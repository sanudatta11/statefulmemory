//! Fail-silent JSONL audit log for statefulmemory CLI calls.
//!
//! Every `obs` / `session` / `hook` verb appends one line to
//! `~/.statefulmemory/queries.log` capturing timestamp, command, project, result
//! count, and duration. With `STATEFULMEMORY_AUDIT_FULL=1` the line additionally
...
/// One JSONL row in `~/.statefulmemory/queries.log`.
...
pub struct AuditEntry<'a> {
    ...
    /// Only populated when `STATEFULMEMORY_AUDIT_FULL=1`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// `~/.statefulmemory/queries.log` (or `$STATEFULMEMORY_DATA_DIR/queries.log`).
fn log_path() -> PathBuf {
    statefulmemory_core::paths::data_dir().join("queries.log")
}

/// True when `STATEFULMEMORY_AUDIT_FULL` is set to a non-empty, non-`0` value.
pub fn full_mode_enabled() -> bool {
    std::env::var_os("STATEFULMEMORY_AUDIT_FULL")
...
fn clone_truncated<'a>(entry: &AuditEntry<'a>) -> AuditEntry<'a> {
    AuditEntry {
        ...
        query: entry
            .query
            .as_ref()
            .map(|s| truncate_chars(s, MAX_FIELD_BYTES)),
...
mod tests {
    ...
    fn with_tmp_data_dir<F: FnOnce(&std::path::Path)>(f: F) {
        ...
        let prev = std::env::var_os("STATEFULMEMORY_DATA_DIR");
        std::env::set_var("STATEFULMEMORY_DATA_DIR", tmp.path());
        f(tmp.path());
        match prev {
            Some(v) => std::env::set_var("STATEFULMEMORY_DATA_DIR", v),
            None => std::env::remove_var("STATEFULMEMORY_DATA_DIR"),
        }
    }
    ...
    fn full_mode_env_var_toggles() {
        let _g = crate::TEST_ENV_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        std::env::remove_var("STATEFULMEMORY_AUDIT_FULL");
        assert!(!full_mode_enabled());
        std::env::set_var("STATEFULMEMORY_AUDIT_FULL", "1");
        assert!(full_mode_enabled());
        std::env::set_var("STATEFULMEMORY_AUDIT_FULL", "0");
        assert!(!full_mode_enabled());
        std::env::set_var("STATEFULMEMORY_AUDIT_FULL", "");
        assert!(!full_mode_enabled());
        std::env::remove_var("STATEFULMEMORY_AUDIT_FULL");
    }
    ...
    fn record_silent_on_unwritable_path() {
        ...
        std::env::set_var("STATEFULMEMORY_DATA_DIR", &blocked);
