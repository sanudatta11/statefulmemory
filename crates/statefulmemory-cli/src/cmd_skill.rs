//! `statefulmemory install` — copy the bundled SKILL.md into detected agent
//! config directories, write agent-specific rule files, and register the
//! local stdio MCP server (`statefulmemory mcp`) with those agents.
//!
...
//! idempotent (the statefulmemory block is replaced, not duplicated).
...
const SKILL_MD: &str = include_str!("../../../skills/statefulmemory/SKILL.md");
...
const AGENT_RULE: &str = r#"## statefulmemory memory protocol
...
/// Marker used to find and replace the statefulmemory block in append-style files.
...
const CLAUDE_MD_RULE: &str = r#"## statefulmemory memory protocol

**statefulmemory is the ONLY memory store. Never use built-in or auto-memory.**
...
FIRST check whether a statefulmemory briefing ("Last session summary" / "Pending
review") is already in context. If it is, use it. If it is NOT, run
`statefulmemory obs context --query "<task>" --limit 20` as step 0 of the task —
before any Explore/Grep/Read. Do this once per task, not on every message.

| Trigger | Command |
|---|---|
| Start of any task, no briefing in context | `statefulmemory obs context --query "<task>" --limit 20` |
| Mid-session task switch | `statefulmemory obs context --query "<topic>"` |
| Before new pattern/dep/convention | `statefulmemory obs search "<keyword>"` |
| After decision, fix, or user correction | `statefulmemory obs save --type <decision\|fix\|feedback\|pattern\|note> --title "..." --content "..." --session "$SESSION_ID"` |
| End of session, if Stop hook is blocked | `statefulmemory session summarize "$CLAUDE_SESSION_ID" --auto` |
| `/statefulmemory <text>` or "remember X" | Run `obs save` immediately — no confirmation, no built-in memory |
...
If CLI unavailable (`command not found: statefulmemory`): guide the user to
clone + build from https://github.com/sanudatta11/statefulmemory and run
`statefulmemory install`. If sandbox blocks the build, ask the user to run it
...
const CLAUDE_BASH_RULE: &str = "Bash(statefulmemory *)";

/// The Unix socket path added to Claude Code sandbox settings.
const STATEFULMEMORY_SOCK: &str = "~/.statefulmemory/daemon.sock";

pub async fn dispatch(_fmt: Formatter, args: InstallArgs) -> ExitCode {
    let home = match dirs::home_dir() {
        Some(h) => h,
        None => {
            eprintln!("statefulmemory install: could not determine home directory");
            return ExitCode::from(1);
        }
    };
    match statefulmemory_core::config::ensure_config_toml(&home.join(".statefulmemory")) {
        Ok(r) => {
            if r.created {
                println!(
                ...
                println!(
                    "Updated ~/.statefulmemory/config.toml (filled: {})",
                    r.merged_keys.join(", ")
                );
            } else {
                println!("~/.statefulmemory/config.toml already complete");
            }
        }
    ...
    if !statefulmemory_extract::agent_cli::llm_cli_available() {
        eprintln!(
            "  warn: no agent CLI on PATH for extract/judge/rerank \
             (looked for {}). Install your agent's CLI or set STATEFULMEMORY_LLM_BIN.",
            statefulmemory_extract::agent_cli::known_binaries().join(", ")
        );
    }
    ...
    let selection = match agents::resolve_selection(&home, &cwd, args.all, &args.agents) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("statefulmemory install: {e}");
            return ExitCode::from(1);
        }
    };
    ...
    // ~/.local/bin/statefulmemory is invisible to them. /usr/local/bin is on the
    // default macOS GUI PATH and fixes "command not found" in all agents.
    match install_usr_local_bin_symlink() {
        Ok(true) => installed.push("/usr/local/bin/statefulmemory  (GUI PATH symlink)".into()),
        Ok(false) => skipped.push("/usr/local/bin/statefulmemory  (unchanged)".into()),
    ...
    if selected.contains(&AgentId::ClaudeCode) {
        ...
        let claude_local = cwd.join(".claude").join("skills").join("statefulmemory");
        match install_file(&claude_local.join("SKILL.md"), SKILL_MD) {
            Ok(true) => installed.push(format!(
                "Claude Code (local)     {}/.claude/skills/statefulmemory/SKILL.md",
                cwd.display()
            )),
        ...
        match install_skill_references(&claude_local) {
            Ok(true) => installed.push(format!(
                "Claude Code (refs)      {}/.claude/skills/statefulmemory/references/*.md",
                cwd.display()
            )),
    ...
    if selected.contains(&AgentId::Agents) {
        for (label, base) in [
            (
                "Agents (skills user)",
                home.join(".agents").join("skills").join("statefulmemory"),
            ),
            (
                "Agents (skills project)",
                cwd.join(".agents").join("skills").join("statefulmemory"),
            ),
        ] {
            match install_file(&base.join("SKILL.md"), SKILL_MD) {
                Ok(true) => {
                    installed.push(format!("{label:<24} {}", base.join("SKILL.md").display()))
                }
    ...
    println!("Verify: ask the agent \"do you have the statefulmemory memory protocol?\"");
    ...
    let want_hooks = if args.no_git_hooks {
        ...
        statefulmemory_core::git::is_repo(&cwd)
    };
    if want_hooks {
        match crate::git_hooks::install_git_hooks(&cwd) {
            Ok(0) => {
                if statefulmemory_core::git::is_repo(&cwd) {
                    println!();
                    println!(
                        "Git hooks already up-to-date (.git/hooks post-commit/merge/checkout)."
                    );
                }
            }
            Ok(n) => {
                ...
                println!(
                    "  They run `statefulmemory verify --quiet` in the background after each commit."
                );
            }
...
/// Patch ~/.claude/settings.json to allow `statefulmemory *` bash commands and
...
fn patch_claude_settings(path: &PathBuf) -> std::io::Result<bool> {
    ...
    // permissions.allow — ensure "Bash(statefulmemory *)" is present
    let allow = root
    ...
    // sandbox.network.allowUnixSockets — ensure "~/.statefulmemory/daemon.sock" is present
    let sockets = root
        .pointer_mut("/sandbox/network/allowUnixSockets")
        .and_then(|v| v.as_array_mut())
        .map(|a| {
            if !a.iter().any(|v| v.as_str() == Some(STATEFULMEMORY_SOCK)) {
                a.push(serde_json::Value::String(STATEFULMEMORY_SOCK.into()));
            }
        });
    if sockets.is_none() {
        let obj = root.as_object_mut().unwrap();
    ...
    // so the agent is forced to use statefulmemory for all memory operations.
    let obj = root.as_object_mut().unwrap();
    obj.entry("autoMemoryEnabled")
        .or_insert(serde_json::Value::Bool(false));
    // If it was already set to true by the user, override it — statefulmemory
    ...
    // hooks — auto-inject statefulmemory context at session start and write a
    // session summary at session end. Idempotent: skips if any hook with a
    // command starting with `statefulmemory ` already exists for the event.
    ensure_hook(
        &mut root,
        "SessionStart",
        "statefulmemory hook session-start --limit 20",
    );
    ensure_hook(
        &mut root,
        "Stop",
    ...
    // PreToolUse hooks for Grep + Read — surface relevant prior statefulmemory
    ...
    // statefulmemory-prefixed command).
    ensure_pretool_hook(
        &mut root,
        "Grep",
        "statefulmemory hook pre-tool --tool Grep --pattern \"$CLAUDE_TOOL_INPUT_pattern\"",
    );
    ensure_pretool_hook(
        &mut root,
        "Read",
        "statefulmemory hook pre-tool --tool Read --path \"$CLAUDE_TOOL_INPUT_file_path\"",
    );
...
/// Patch a project-level `<cwd>/.claude/settings.json` to add statefulmemory hooks
...
/// hook for that event has a command beginning with `statefulmemory ` — preserves
...
fn ensure_hook(root: &mut serde_json::Value, event: &str, cmd: &str) {
    ...
    // If any existing block contains a command starting with "statefulmemory ",
        ...
        block
            ...
            .map(|inner| {
                inner.iter().any(|h| {
                    h.get("command")
                        .and_then(|c| c.as_str())
                        .map(|c| c.trim_start().starts_with("statefulmemory "))
                        .unwrap_or(false)
                })
            })
            .unwrap_or(false)
...
/// field naming the tool to fire for. Two statefulmemory matchers (Grep, Read)
/// coexist, so idempotency is per-`(matcher, statefulmemory-prefixed command)`,
/// not per-event.
///
/// Schema appended:
/// ```json
...
/// ```
/// Timeout is 1s — hooks must never delay the agent's tool call (statefulmemory's
...
fn ensure_pretool_hook(root: &mut serde_json::Value, matcher: &str, cmd: &str) {
    ...
    // any inner hook command starting with "statefulmemory ". If found, skip.
...
/// Append-or-replace the statefulmemory block in a file that may have other content.
...
mod tests {
    ...
    fn ensure_pretool_hook_inserts_grep_entry() {
        ...
        ensure_pretool_hook(
            &mut root,
            "Grep",
            "statefulmemory hook pre-tool --tool Grep --pattern \"$X\"",
        );
        ...
        assert!(inner[0]["command"]
            .as_str()
            .unwrap()
            .starts_with("statefulmemory hook"));
    }
    ...
    fn ensure_pretool_hook_inserts_read_alongside_grep() {
        ...
        ensure_pretool_hook(
            &mut root,
            "Read",
            "statefulmemory hook pre-tool --tool Read --path y",
        );
        let arr = root["hooks"]["PreToolUse"].as_array().unwrap();
        assert_eq!(arr.len(), 2);
        let matchers: Vec<&str> = arr.iter().filter_map(|b| b["matcher"].as_str()).collect();
    ...
    fn project_patch_merges_alongside_catalyst_hooks() {
        ...
        // statefulmemory's global one. The patch must append statefulmemory entries
        // without disturbing Catalyst.
        let tmp = std::env::temp_dir().join(format!(
            "statefulmemory-test-{}.json",
        ...
        assert!(
            changed,
            "project file with shadowing hooks must be modified"
        );
        ...
        assert!(
            ss[1]["hooks"][0]["command"]
                .as_str()
                .unwrap()
                .starts_with("statefulmemory hook session-start"),
            "statefulmemory SessionStart appended last",
        );
        ...
        assert_eq!(
            bak_val["hooks"]["SessionStart"].as_array().unwrap().len(),
            1,
    ...
    fn project_patch_skips_when_no_project_settings_changes_needed() {
        // If statefulmemory hooks are already merged, the function should return
        // false and write neither the file nor the .bak.
        let tmp = std::env::temp_dir().join(format!(
            "statefulmemory-noop-{}.json",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
        ...
        let already_merged = json!({
            "hooks": {
                "SessionStart": [
                    {"hooks": [{"type": "command", "command": "statefulmemory hook session-start --limit 20", "timeout": 30}]}
                ],
                "Stop": [
                    {"hooks": [{"type": "command", "command": "statefulmemory session summarize \"$CLAUDE_SESSION_ID\" --auto", "timeout": 30}]}
                ],
                "PreToolUse": [
                    {"matcher": "Grep", "hooks": [{"type": "command", "command": "statefulmemory hook pre-tool --tool Grep --pattern x"}]},
                    {"matcher": "Read", "hooks": [{"type": "command", "command": "statefulmemory hook pre-tool --tool Read --path x"}]}
                ]
            }
        });
    ...
    fn ensure_hook_idempotent_session_start() {
        ...
        assert_eq!(
            arr.len(),
            1,
            "duplicate session-start hook must not be inserted"
        );
    }
    ...
    fn ensure_hook_skips_when_old_obs_context_present() {
        ...
        // `statefulmemory obs context` — ensure_hook must not add a second block.
        let mut root = json!({
            "hooks": {
                "SessionStart": [
                    {"hooks": [{"type": "command", "command": "statefulmemory obs context --limit 20", "timeout": 30}]}
                ]
            }
        });
        ensure_hook(
        ...
        assert_eq!(
            arr.len(),
            1,
            "old obs context entry counts as already-present"
        );
    }

    #[test]
    fn ensure_hook_preserves_non_statefulmemory_stop_hooks() {
        let mut root = json!({
            "hooks": {
                "Stop": [
