...
//!
//! Each verb function takes a connected `StatefulMemoryClient<Channel>`, the
//! resolved project name, the chosen [`Formatter`], and the verb-specific
//! args. It builds the matching `statefulmemory_proto` request, awaits the RPC,
...
pub type Client = p::stateful_memory_client::StatefulMemoryClient<Channel>;
...
async fn capture_passive(
    ...
    let response = p::CapturePassiveResponse {
        snippets: resp.snippets,
    };
    let combined = crate::render::CapturePassiveOutcome { response, saved };
...
async fn history(
    ...
    let observation_id: i64 = a.id.parse().map_err(|_| {
        VerbError::Usage(format!("obs history: id must be numeric (got {:?})", a.id))
    })?;
