...
//! (spec2-t5).
//!
//! The proto types in `statefulmemory-proto` are not `serde::Serialize` (prost is
...
fn write_observation_detail(o: &p::Observation, w: &mut dyn Write) -> io::Result<()> {
    ...
    if !o.supersedes_ids.is_empty() {
        ...
        writeln!(
            w,
            "current — supersedes #{} (use `statefulmemory obs history {}` for the chain)",
            ids.join(", #"),
            o.id
        )?;
    }
...
fn write_observation_row(o: &p::Observation, w: &mut dyn Write) -> io::Result<()> {
        ...
        .filter(|s| matches!(*s, "stale" | "invalidated" | "unprovable"))
    {
        writeln!(
            w,
            "         [{vs}] — withdrawn from context; run `statefulmemory verify`"
        )?;
    }
    if !o.supersedes_ids.is_empty() {
        let ids: Vec<String> = o.supersedes_ids.iter().map(|id| id.to_string()).collect();
