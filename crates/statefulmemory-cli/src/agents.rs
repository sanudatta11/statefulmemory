//! Agent detection for `statefulmemory install`, mirroring skills.sh / `npx skills`.
//!
    ...
    let empty_hint = if selected.is_empty() {
        Some(
            "No coding agents detected. Install an agent, or re-run with \
             `statefulmemory install --all` or `statefulmemory install --agent cursor`.",
        )
    } else {
        None
    };
...
fn is_installed(id: AgentId, home: &Path, cwd: &Path) -> bool {
    match id {
        ...
        AgentId::Cursor => {
            home.join(".cursor").is_dir() || path_on_path("cursor") || env_truthy("CURSOR_TRACE_ID")
        }
        ...
        AgentId::KimiCode => home.join(".kimi-code").is_dir() || home.join(".kimi").is_dir(),
        ...
        AgentId::GeminiCli => {
            home.join(".gemini").is_dir() || path_on_path("gemini") || env_truthy("GEMINI_CLI")
        }
        AgentId::Codex => {
            home.join(".codex").is_dir() || path_on_path("codex") || env_truthy("CODEX_SANDBOX")
        }
