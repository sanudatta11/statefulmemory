//! `statefulmemory eval` — benchmark evaluation and scorecard reporting.

use std::path::PathBuf;
use std::process::ExitCode;

use statefulmemory_eval::config::{default_profile, RetrievalConfig, RetrievalMode};
use statefulmemory_eval::datasets::{EvalMemory, EvalQuery};
use statefulmemory_eval::{BenchmarkKind, RunConfig, Scorecard};
...
async fn run_eval(args: EvalArgs, output_fmt: Option<OutputFormat>) -> Result<(), String> {
    ...
    let report = statefulmemory_eval::runner::run(&cfg, memories, queries)
        .await
...
fn load_dataset(
    ...
    match kind {
        BenchmarkKind::Locomo => {
            let (m, q) = statefulmemory_eval::datasets::locomo::load(data_dir)
                .map_err(|e| format!("load LoCoMo: {e:#}"))?;
            Ok((m, statefulmemory_eval::apply_query_limit(q, limit, true)))
        }
        BenchmarkKind::Longmemeval => {
            let (m, q) = statefulmemory_eval::datasets::longmemeval::load(data_dir)
                .map_err(|e| format!("load LongMemEval: {e:#}"))?;
            Ok((m, statefulmemory_eval::apply_query_limit(q, limit, false)))
        }
        BenchmarkKind::Beam1m => {
            let cap = limit.unwrap_or(usize::MAX);
            let (m, q) = statefulmemory_eval::datasets::beam::generate_all(
                statefulmemory_eval::datasets::beam::BeamScale::M1,
                cap.min(1000),
            )
        ...
        BenchmarkKind::Beam10m => {
            Err("beam-10m requires `eval prepare` first; use the eval binary or --smoke".into())
        }
