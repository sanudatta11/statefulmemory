...
async fn stop(_fmt: Formatter) -> ExitCode {
    ...
    if !socket.exists() {
        eprintln!(
            "statefulmemory: daemon is not running (no socket at {})",
            socket.display()
        );
        return ExitCode::from(exit::ALREADY_IN_STATE);
    }
    ...
    if socket.exists() {
        if let Ok(c) = open_client_no_spawn(&socket).await {
            ...
            if let Err(s) = client.shutdown(p::ShutdownRequest {}).await {
                eprintln!("statefulmemory: stop during restart: {}", s.message());
...
async fn force_start(fmt: Formatter) -> ExitCode {
    ...
    if cfg.socket.exists() {
        if let Err(e) = std::fs::remove_file(&cfg.socket) {
            eprintln!("statefulmemory: failed to remove stale socket: {e}");
...
async fn open_client_no_spawn(socket: &std::path::Path) -> Result<Client, ExitCode> {
    ...
    match client_channel::connect_uds(socket) {
        Ok(c) => Ok(StatefulMemoryClient::new(c)),
        Err(ClientError::SocketNotFound(_)) => {
            eprintln!(
                "statefulmemory: daemon socket disappeared at {}",
                socket.display()
            );
