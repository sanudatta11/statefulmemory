...
/// naive (basename only) and does not honor `.statefulmemory/config.json`,
/// `[remote "origin"]`, `--project`, or `STATEFULMEMORY_PROJECT`. The CLI has
...
async fn consolidate(
    ...
    if !a.dry_run && !resp.candidates.is_empty() {
        eprintln!(
            "statefulmemory: pass --dry-run to confirm; interactive merge of {} candidate(s) requires the user to invoke `project merge` for each pair",
            resp.candidates.len()
        );
    }
    Ok(())
...
mod tests {
    ...
    fn current_local_renders_from_detection_no_rpc() {
        ...
        // `STATEFULMEMORY_PROJECT`, and `.statefulmemory/config.json`.
        let cases: Vec<(ProjectSource, &str)> = vec![
            (ProjectSource::CliFlag, "cli_flag"),
            (ProjectSource::EnvOverride, "env_override"),
            (
                ProjectSource::ConfigFile(PathBuf::from("/tmp/.statefulmemory/config.json")),
                "config_file",
            ),
            (
                ProjectSource::GitRemote("git@github.com:acme/widgets.git".into()),
                "git_remote",
            ),
            (
                ProjectSource::GitRoot(PathBuf::from("/tmp/widgets")),
                "git_root_basename",
            ),
        ];
    ...
    fn merge_rejects_empty_from_or_to() {
        let a = ProjectMergeArgs {
            from: "  ".into(),
            to: "beta".into(),
        };
        assert!(validate_merge(&a).is_err());
        let b = ProjectMergeArgs {
            from: "alpha".into(),
            to: "".into(),
        };
    ...
    fn dry_run_does_not_mutate() {
        ...
        let cli = Cli::try_parse_from(["statefulmemory", "project", "prune", "--dry-run"]).unwrap();
        match cli.command {
            Command::Project(ProjectArgs {
        ...
        let cli = Cli::try_parse_from(["statefulmemory", "project", "consolidate"]).unwrap();
        match cli.command {
            Command::Project(ProjectArgs {
                verb: ProjectVerb::Consolidate(a),
