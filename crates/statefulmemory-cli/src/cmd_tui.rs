...
pub async fn run(
    client: &mut StatefulMemoryClient<Channel>,
    project_name: &str,
        ...
        terminal.draw(|f| {
            ...
            let search_bar = Paragraph::new(format!(" Search: {}", search_query))
                ...
                .block(Block::default().borders(Borders::ALL).title(format!(
                    " statefulmemory observation browser — project: [{}] ",
                    project_name
                )));
            ...
            let selected_obs = list_state.selected().and_then(|idx| observations.get(idx));
                ...
                match input_mode {
                    InputMode::Normal => match key.code {
                        ...
                        KeyCode::Char('/') | KeyCode::Tab => {
                            ...
                            status_msg = String::from(
                                "SEARCH MODE: Type query, Press Enter or Esc to finish",
                            );
                        }
                            ...
                            if let Some(idx) = list_state.selected() {
                                if let Some(obs) = observations.get(idx) {
                                    let req = DeleteObservationRequest {
                                        project_name: project_name.to_string(),
                                        key: Some(
                                            statefulmemory_proto::delete_observation_request::Key::Id(
                                                obs.id,
                                            ),
                                        ),
                                        hard: false,
                                    ...
                                    if client.delete_observation(req).await.is_ok() {
                                        status_msg =
                                            format!("Soft deleted observation #{}", obs.id);
                                        observations =
                                            fetch_observations(client, project_name, &search_query)
                                                .await;
                    ...
                    InputMode::Search => match key.code {
                        KeyCode::Enter | KeyCode::Esc => {
                            ...
                            status_msg =
                                String::from("Press 'q' to quit, '/' to search, 'd' to delete");
                        }
                        KeyCode::Backspace => {
                            search_query.pop();
                            observations =
                                fetch_observations(client, project_name, &search_query).await;
                            list_state.select(if observations.is_empty() {
                                None
                            } else {
                                Some(0)
                            });
