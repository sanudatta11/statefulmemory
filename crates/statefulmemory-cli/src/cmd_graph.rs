...
//! `statefulmemory graph query|rebuild|stats` — entity graph CLI (spec: graph-briefing).
//!
...
async fn query(
    ...
    let entity_id = pick_entity_id(&list.entities, &a.entity).ok_or_else(|| {
        VerbErr::Usage(format!(
            "error: entity not found: \"{}\" (run `statefulmemory graph rebuild` first)",
            a.entity
        ))
    })?;
...
fn project_db_path(project_name: &str) -> std::path::PathBuf {
    statefulmemory_core::paths::project_db_path(project_name)
}
...
fn rebuild(project_name: &str, quiet: bool) -> Result<(), VerbErr> {
    ...
    if !graph_enabled_in_config(project_name) && !quiet {
        eprintln!(
            "note: graph disabled in config; enable with `statefulmemory config set graph.enabled true`"
        );
    }
    Ok(())
}
...
fn graph_enabled_in_config(project_name: &str) -> bool {
    ...
    for path in [
        statefulmemory_core::config::statefulmemory_global_config_path(),
        statefulmemory_core::config::statefulmemory_project_config_path(project_name),
    ] {
