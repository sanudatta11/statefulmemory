...
/// `Self` because the prost-generated proto types in `statefulmemory-proto` are
...
mod tests {
    ...
    fn explicit_output_wins_regardless_of_tty() {
        assert_eq!(
            Formatter::resolve(Some(Formatter::Yaml), true),
            Formatter::Yaml
        );
        assert_eq!(
            Formatter::resolve(Some(Formatter::Json), true),
            Formatter::Json
        );
        assert_eq!(
            Formatter::resolve(Some(Formatter::Text), false),
            Formatter::Text
        );
    }
