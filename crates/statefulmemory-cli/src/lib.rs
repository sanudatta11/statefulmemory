//! statefulmemory CLI: clap dispatch, output formatting, project detection,
...
//! `STATEFULMEMORY_PROJECT` env (FR3.4), `--quiet` (FR1.6), auto-spawn (FR2),
//! exit codes (FR13) — live here so each command module added in spec2-t5..t7
//! stays focused on a single RPC.

use std::sync::atomic::{AtomicBool, Ordering};

/// Shared by unit tests that mutate process-global `STATEFULMEMORY_*` / `NO_COLOR` env.
