...
//!
//! Prints `statefulmemory X.Y.Z (commit <sha>, built <date>)`. The git SHA and
...
pub fn version_string() -> String {
    let sha: &str = env!("STATEFULMEMORY_GIT_SHA");
    let ts: i64 = env!("STATEFULMEMORY_BUILD_TS").parse().unwrap_or(0);
    let date = Utc
        .timestamp_opt(ts, 0)
        .single()
    ...
    format!(
        "statefulmemory {} (commit {sha}, built {date})",
...
mod tests {
    ...
    fn version_format_matches_regex() {
        // SC-24: `statefulmemory [0-9]+\.[0-9]+\.[0-9]+ \(commit [0-9a-f]+`. We
        // implement the regex check by hand to avoid a `regex` dev-dep.
        let s = version_string();
        assert!(
            s.starts_with("statefulmemory "),
            "missing 'statefulmemory ' prefix: {s}"
        );
        ...
        // Pull out the version segment between "statefulmemory " and " (commit ".
        let after_prefix = s.strip_prefix("statefulmemory ").unwrap();
