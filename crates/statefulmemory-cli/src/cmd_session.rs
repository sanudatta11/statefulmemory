...
async fn summarize(
    ...
    let body: String = match a.content.as_deref() {
        ...
        None => {
            ...
            if resp.observations.is_empty() {
                eprintln!(
                    "statefulmemory: session {session_id} has no observations — nothing to summarize"
                );
                return Ok(());
            }
    ...
    let req = p::SaveObservationRequest {
        ...
        scope: "project".into(),
        created_by: Some(
            if a.auto && a.content.is_none() {
                "statefulmemory-summarize/auto"
            } else {
                "statefulmemory-summarize/agent"
            }
            .into(),
        ),
...
fn render_rollup(session_id: &str, obs: &[p::Observation]) -> String {
    ...
    let section =
        |title: &str, key: &str, by_type: &BTreeMap<&str, Vec<&p::Observation>>| -> String {
