//! `statefulmemory hook pre-tool` — Claude Code PreToolUse hook handler.
//!
...
type Client = p::stateful_memory_client::StatefulMemoryClient<Channel>;
...
const STOPWORDS: &[&str] = &[
    "src", "crates", "lib", "mod", "main", "test", "tests", "index", "util", "utils", "common",
    "shared", "internal", "bin", "pkg", "cmd", "core", "api",
];
...
/// the model's next-turn context, so this adds statefulmemory-resident memory
/// before the Grep result appears.
fn render_grep_markdown(query: &str, observations: &[p::Observation]) {
    println!("## statefulmemory prior context for \"{query}\"");
    println!();
...
fn render_read_hint(query: &str, hit_count: usize) {
    eprintln!(
        "statefulmemory hint: {hit_count} prior observations match \"{query}\" — \
         run `statefulmemory obs search \"{query}\"` for context",
    );
}
...
mod tests {
    ...
    fn keywords_skip_stopwords_and_short_tokens() {
        let kw = keywords_from_path(Path::new("crates/statefulmemory-storage/src/write.rs"));
        // 'src' and 'crates' are stopwords; <3-char tokens skipped.
        assert!(!kw.contains(&"src".to_string()));
        assert!(!kw.contains(&"crates".to_string()));
        // 'statefulmemory-storage' splits into 'statefulmemory' + 'storage'; stem 'write'.
        assert!(
            kw.contains(&"write".to_string()),
            "missing 'write' in {kw:?}"
        );
        assert!(
            kw.contains(&"statefulmemory".to_string()) || kw.contains(&"storage".to_string()),
    ...
    fn first_meaningful_line_trims_long_input() {
        let body =
            "  \n\n   first content line that goes on for a while ".to_string() + &"x".repeat(200);
