...
//!
//! `logs --lines N` prints the last N lines of `~/.statefulmemory/daemon.log`.
...
pub async fn dispatch(args: LogsArgs) -> ExitCode {
    let path = statefulmemory_core::paths::log_path();
    if !path.exists() {
        eprintln!(
            "statefulmemory: log file does not exist at {}; the daemon may not have started yet",
            path.display()
        );
        return ExitCode::from(exit::GENERAL);
    }
    if let Err(e) = print_tail(&path, args.lines) {
        eprintln!("statefulmemory: read {}: {e}", path.display());
        return ExitCode::from(exit::GENERAL);
    }
    if args.follow {
        if let Err(e) = follow(&path).await {
            eprintln!("statefulmemory: follow {}: {e}", path.display());
            return ExitCode::from(exit::GENERAL);
        }
    }
    ExitCode::SUCCESS
