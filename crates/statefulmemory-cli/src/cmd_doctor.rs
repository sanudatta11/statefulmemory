//! CLI command handler for `statefulmemory doctor` (health audit & auto-repair).

use crate::cli::DoctorArgs;
use statefulmemory_client::StatefulMemoryClient;
use statefulmemory_proto::DoctorRequest;
use statefulmemory_storage::doctor::DoctorFinding;
use tonic::transport::Channel;

pub async fn run(
    client: Option<&mut StatefulMemoryClient<Channel>>,
    project_name: &str,
    args: DoctorArgs,
    output_json: bool,
) -> anyhow::Result<()> {
    let findings: Vec<DoctorFinding> = if let Some(c) = client {
        ...
        if !db_path.exists() {
            ...
            let conn = if args.repair {
                statefulmemory_storage::db::open_write(&db_path)?
            } else {
                statefulmemory_storage::db::open_read(&db_path)?
            };
            statefulmemory_storage::doctor::audit_and_repair(&conn, args.repair)?
        }
    };
    ...
    println!("{}", "-".repeat(90));
    // Wave 3 retrieve knobs (local CE + adaptive router).
    let cfg = statefulmemory_core::config::load_resolved(Some(project_name));
    println!(
        "retrieve: search.mode={} search.rerank={} rerank.backend={} search.router={} graph.ranker={}",
        cfg.search.mode,
        cfg.search.rerank,
        cfg.rerank.backend,
        cfg.search.router,
