...
//!    `.statefulmemory/config.json`; return its `project_name`. Source = `config`.
...
//! - `STATEFULMEMORY_PROJECT` env var
//!
...
pub enum ProjectSource {
    ...
    /// `STATEFULMEMORY_PROJECT` env var.
    EnvOverride,
    /// `<repo>/.statefulmemory/config.json`.
    ConfigFile(PathBuf),
...
pub struct ProjectDetection {
    ...
    /// `~/.statefulmemory/projects/<normalized>.db` keys off this.
...
impl ProjectDetection {
    ...
    /// `crates/statefulmemory-tests/tests/cli_format_exitcodes.rs` assert specific
...
pub enum DetectError {
    ...
    #[error(
        "statefulmemory: cannot determine project — current directory is not a git repo and contains no .statefulmemory/config.json. Either run from inside a git repo, create .statefulmemory/config.json with {{\"project_name\":\"<name>\"}}, or pass --project <name>."
    )]
    Ambiguous,
...
/// of `STATEFULMEMORY_PROJECT`. Pass `None` for either when the source is absent.
pub fn detect_in(
    cwd: &Path,
    ...
    if let Some(cfg_path) = find_config_in_ancestors(cwd, git_root.as_deref()) {
        ...
        let cfg: ProjectConfigFile =
            serde_json::from_str(&body).map_err(|e| DetectError::Config {
                path: cfg_path.clone(),
                reason: e.to_string(),
            })?;
...
/// Walk up from `cwd` looking for `.statefulmemory/config.json`. Stops at
...
fn find_config_in_ancestors(cwd: &Path, git_root: Option<&Path>) -> Option<PathBuf> {
    ...
    loop {
        let cfg = cur.join(".statefulmemory").join("config.json");
        if cfg.is_file() {
            return Some(cfg);
        }
...
pub fn normalize_name(input: &str) -> Result<String, DetectError> {
    ...
    if out.is_empty() {
        return Err(DetectError::InvalidName(format!(
            "normalization removed all characters: '{input}'"
        )));
    }
    if out.starts_with('.') {
        return Err(DetectError::InvalidName(format!(
...
mod tests {
    ...
    fn write_config_json(repo: &Path, project_name: &str) {
        fs::create_dir_all(repo.join(".statefulmemory")).unwrap();
        fs::write(
            repo.join(".statefulmemory").join("config.json"),
    ...
    fn parse_remote_handles_https() {
        assert_eq!(
            parse_remote_to_name("https://github.com/acme/widgets.git"),
            Some("widgets".into())
        );
        assert_eq!(
            parse_remote_to_name("https://github.com/acme/widgets"),
            Some("widgets".into())
        );
    }
    ...
    fn parse_remote_handles_ssh() {
        assert_eq!(
            parse_remote_to_name("git@github.com:acme/widgets.git"),
            Some("widgets".into())
        );
        assert_eq!(
            parse_remote_to_name("ssh://git@github.com/acme/widgets.git"),
            Some("widgets".into())
        );
    }
