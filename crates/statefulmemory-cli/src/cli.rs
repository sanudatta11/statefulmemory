//! `clap` derive structures for the full statefulmemory CLI surface (PRD §3.3).
//!
...
#[command(
    ...
    long_about = "statefulmemory stores observations, sessions, and prompts in a per-project SQLite \
...
pub struct Cli {
    ...
    /// `STATEFULMEMORY_PROJECT` env var is honored too, but `main.rs` reads it
...
pub enum Command {
    ...
    /// (`statefulmemory mcp`) for detected coding agents (skills.sh-style).
    /// Use `--all` or `--agent` to override auto-detection.
    Install(InstallArgs),
    /// Remove statefulmemory skill files, settings patches, and MCP registration.
    /// Pass `--purge` to also wipe data dirs, model cache, binaries, and legacy memlayer residue.
    ...
    /// embed). Backed by `~/.statefulmemory/config.toml` (global) and
    /// `~/.statefulmemory/projects/<name>.config.toml` (per-project).
    Config(ConfigArgs),
    /// Re-index all observations into the vector store (`statefulmemory reindex`).
    Reindex(ReindexArgs),
    ...
    Graph(GraphArgs),
    /// Compile a host-window memory brief (`smem context compile`).
    Context(ContextArgs),
    /// Ingest repo / docs into durable memory (`smem ingest repo`).
    Ingest(IngestArgs),
...
pub enum ContextVerb {
    /// Pack durable memory into a slot-aware brief for a host context window.
    Compile(ContextCompileArgs),
}

#[derive(Args, Debug)]
pub struct ContextCompileArgs {
    /// Focus query for retrieval (recommended).
    #[arg(long)]
    pub query: Option<String>,
    /// Host window preset: `8k` | `32k` | `128k` | `200k` | `1m` (or raw integer).
    #[arg(long, default_value = "32k")]
    pub window: Option<String>,
    /// Override token budget (estimated chars/4). Wins over `--window` when set.
    #[arg(long)]
    pub max_tokens: Option<i32>,
    /// Candidate pool size before slot packing.
    ...
    /// Skip GetFacts RPCs for packed observations.
    #[arg(long)]
    pub no_facts: bool,
}
...
#[derive(Subcommand, Debug)]
pub enum IngestVerb {
    /// Index git-tracked text files as path-anchored observations.
    Repo(IngestRepoArgs),
}

#[derive(Args, Debug)]
pub struct IngestRepoArgs {
    /// Git work-tree root (default: cwd).
    #[arg(long)]
    pub path: Option<std::path::PathBuf>,
    /// Max file size in bytes (default 262144).
    #[arg(long)]
    pub max_bytes: Option<u64>,
    /// Comma-separated extensions (default: rs,md,toml,py,ts,…).
    #[arg(long)]
    pub ext: Option<String>,
    /// Session id stamped on saved observations.
    #[arg(long)]
    pub session: Option<String>,
    /// List files that would be saved without writing.
    #[arg(long)]
    pub dry_run: bool,
}
...
pub struct UninstallArgs {
    /// Also wipe data dirs (`~/.statefulmemory`, `~/.memlayer`), model caches,
    /// installed binaries (`statefulmemory`/`smem`/`memlayer`), MCP keys, skills,
    /// and git-hook markers for both brands. Destructive; requires confirmation.
    #[arg(long)]
    pub purge: bool,
}
...
pub struct InstallArgs {
    ...
    /// `statefulmemory verify --quiet`. Default: on when cwd is a git repo.
    #[arg(long)]
    pub git_hooks: bool,
...
// -----------------------------------------------------------------------------
// `statefulmemory config` — view / edit retrieval-pipeline tunables (rp-t11).
// -----------------------------------------------------------------------------
...
pub enum ConfigVerb {
    ...
    Show(ConfigShowArgs),
    /// Print one resolved value, e.g. `statefulmemory config get extract.model`.
    Get(ConfigGetArgs),
...
pub struct ConfigShowArgs {
    ...
    /// `~/.statefulmemory/projects/<name>.config.toml` on top of the global
...
pub struct ConfigSetArgs {
    ...
    /// `~/.statefulmemory/projects/<name>.config.toml` instead of the global one.
    #[arg(long)]
    pub project: Option<String>,
}
...
mod tests {
    ...
    fn hook_session_start_parses_with_default_limit() {
        let cli = Cli::try_parse_from(["statefulmemory", "hook", "session-start"]).unwrap();
        match cli.command {
    ...
    fn daemon_force_start_parses() {
        let cli = Cli::try_parse_from(["statefulmemory", "daemon", "force-start"]).unwrap();
        match cli.command {
            Command::Daemon(DaemonArgs {
    ...
    fn project_flag_parses_without_env_binding() {
        ...
        // `main.rs` can read STATEFULMEMORY_PROJECT separately and attribute the
    ...
    fn parse_obs_search(args: &[&str]) -> ObsSearchArgs {
        let mut full = vec!["statefulmemory", "obs", "search"];
    ...
    fn obs_search_invalid_rerank_rejected() {
        let res = Cli::try_parse_from(["statefulmemory", "obs", "search", "q", "--rerank", "opus"]);
    ...
    fn obs_facts_verb_parses() {
        let cli = Cli::try_parse_from(["statefulmemory", "obs", "facts", "42"]).unwrap();
        match cli.command {
            Command::Obs(o) => match o.verb {
    ...
    fn obs_history_verb_parses() {
        let cli = Cli::try_parse_from(["statefulmemory", "obs", "history", "99"]).unwrap();
        match cli.command {
            Command::Obs(o) => match o.verb {
    ...
    fn daemon_restart_still_parses() {
        let cli = Cli::try_parse_from(["statefulmemory", "daemon", "restart"]).unwrap();
        assert!(matches!(
            cli.command,
            Command::Daemon(DaemonArgs {
    ...
    fn daemon_stop_still_parses() {
        let cli = Cli::try_parse_from(["statefulmemory", "daemon", "stop"]).unwrap();
        assert!(matches!(
            cli.command,
            Command::Daemon(DaemonArgs {
    ...
    fn daemon_status_still_parses() {
        let cli = Cli::try_parse_from(["statefulmemory", "daemon", "status"]).unwrap();
        assert!(matches!(
            cli.command,
            Command::Daemon(DaemonArgs {
