//! `statefulmemory mem export|import` — portable `.mem` archives.
...
use statefulmemory_sync::mem_archive;
...
note: archive is not seed-encrypted (obfuscated only).\n\
      encrypt with a seed phrase (same phrase required on import):\n\
        statefulmemory mem export --out FILE.mem --seed-file ./phrase.txt\n\
        statefulmemory mem export --out FILE.mem --seed-phrase 'your phrase here'\n\
...
error: this archive is seed-encrypted and cannot be imported without the seed phrase.
       pass the same phrase used at export:
         statefulmemory mem import FILE.mem --seed-file ./phrase.txt
         statefulmemory mem import FILE.mem --seed-phrase 'your phrase here'";
...
async fn import(
    ...
    let encrypted = statefulmemory_archive_encrypted(&bytes, &file)?;
    let seed_phrase = load_seed(a.seed_phrase, a.seed_file.as_deref())?;
...
fn statefulmemory_archive_encrypted(bytes: &[u8], path: &Path) -> Result<bool, VerbErr> {
    mem_archive::is_encrypted(bytes).map_err(|e| match e {
        statefulmemory_sync::SyncError::NotArchive => VerbErr::Usage(format!(
            "error: {} is not a statefulmemory archive",
            path.display()
        )),
