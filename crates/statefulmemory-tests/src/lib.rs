//! Integration-test harness for the `statefulmemory` CLI binary and daemon.
//!
...
///
/// The repo checkout is often named `statefulmemory` (e.g. GHA
/// `/home/runner/work/statefulmemory/statefulmemory`). Walking parents with
...
pub fn locate_binary() -> PathBuf {
    if let Ok(p) = std::env::var("CARGO_BIN_EXE_statefulmemory") {
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        // the sibling `statefulmemory` next to `deps/`.
        if let Some(deps) = exe.parent() {
            if deps.file_name().and_then(|s| s.to_str()) == Some("deps") {
                let cand = deps.join("../statefulmemory");
                if let Ok(canon) = cand.canonicalize() {
                    if is_cli_binary(&canon) {
                        return canon;
                    }
                }
                let cand = deps.parent().map(|p| p.join("statefulmemory"));
                if let Some(c) = cand {
                    if is_cli_binary(&c) {
                        return c;
                    }
        ...
        for _ in 0..6 {
            ...
            let cand = dir.join("statefulmemory");
    ...
    for profile in ["debug", "release"] {
        let cand = target.join(profile).join("statefulmemory");
        if is_cli_binary(&cand) {
            return cand;
        }
    }

    // Auto-build `statefulmemory-cli` on demand if binary does not exist yet.
    let status = Command::new("cargo")
        .args(["build", "-p", "statefulmemory-cli"])
        .status();
    if let Ok(st) = status {
        if st.success() {
            for profile in ["debug", "release"] {
    ...
    panic!(
        "could not locate `statefulmemory` binary; tried CARGO_BIN_EXE_statefulmemory, \
         walking up from current_exe(), and {target_debug}/statefulmemory / \
         {target_release}/statefulmemory. Run `cargo build -p statefulmemory-cli` first.",
...
pub async fn connect(handle: &DaemonHandle) -> StatefulMemoryClient<Channel> {
...
async fn try_connect(sock: &Path) -> Result<StatefulMemoryClient<Channel>, tonic::transport::Error> {
...
///
/// Each `CliEnv` owns a tempdir set as `STATEFULMEMORY_DATA_DIR` so the daemon
/// stores its socket, log, project DBs, and tokens inside the tempdir. The
/// tempdir is cleaned up when the env is dropped.
///
/// Default project name is `test-proj` (set via `STATEFULMEMORY_PROJECT`) so tests
...
impl CliEnv {
    ...
    pub fn spawn_daemon(&mut self) -> &mut Self {
        ...
        let child = Command::new(&self.binary)
            .args(["daemon", "start", "--foreground"])
            .env("STATEFULMEMORY_DATA_DIR", self.data_path())
            .env("STATEFULMEMORY_LOG", "warn")
    ...
    pub fn cmd(&self) -> Command {
        ...
        c.env_clear()
            ...
            .env("STATEFULMEMORY_DATA_DIR", self.data_path())
            .env("STATEFULMEMORY_PROJECT", &self.project)
            .env("STATEFULMEMORY_LOG", "warn")
...
///
/// Spawns the daemon binary with `STATEFULMEMORY_LISTEN=tcp://127.0.0.1:<port>`,
...
impl TcpDaemon {
    ...
    pub fn generate_certs(data_dir: &Path) -> (PathBuf, PathBuf, Vec<u8>) {
        ...
        ca_params
            .distinguished_name
            .push(DnType::CommonName, "statefulmemory-test-ca");
    ...
    pub fn spawn(data_dir: tempfile::TempDir) -> Self {
        ...
        let _guard = TCP_SPAWN_LOCK.lock().unwrap_or_else(|p| p.into_inner());
        ...
        for attempt in 1..=5 {
            ...
            let stderr_log = std::fs::File::create(&stderr_path).expect("create daemon.stderr log");
            let daemon = Command::new(&binary)
                .args(["daemon", "start", "--foreground"])
                .env("STATEFULMEMORY_DATA_DIR", data_dir.path())
                .env("STATEFULMEMORY_LISTEN", &listen)
                .env("STATEFULMEMORY_TLS_CERT", &cert)
                .env("STATEFULMEMORY_TLS_KEY", &key)
                .env("STATEFULMEMORY_LOG", "warn")
                .env("RUST_BACKTRACE", "1")
                .stdout(Stdio::null())
                .stderr(stderr_log)
...
mod locate_binary_tests {
    ...
    #[test]
    fn rejects_checkout_directory_named_statefulmemory() {
        let td = tempfile::TempDir::new().unwrap();
        // Mimic GHA layout: …/work/statefulmemory/statefulmemory (directory, not the CLI).
        let checkout = td.path().join("statefulmemory");
        ...
        let fake_bin = td.path().join("bin").join("statefulmemory");
        fs::create_dir_all(fake_bin.parent().unwrap()).unwrap();
        fs::write(&fake_bin, b"#!/bin/true\n").unwrap();
        assert!(is_cli_binary(&fake_bin));
        assert!(!is_cli_binary(&PathBuf::from("/no/such/statefulmemory")));
    }
}
