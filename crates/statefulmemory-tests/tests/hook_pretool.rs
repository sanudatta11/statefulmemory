//! `statefulmemory hook pre-tool` end-to-end integration tests (aid-t9, aid-t10, aid-t11).
//!
//! Spec coverage:
...
//!   SC-3: statefulmemory install adds exactly one PreToolUse entry per matcher
...
fn grep_hook_returns_prior_observation() {
    ...
    let _ = env
        ...
        .args([
            ...
            "use pgx not gorm",
            "--content",
            "Team prefers raw SQL via pgx; GORM rejected for codegen overhead",
            "--session",
            &session,
        ])
        .output()
    ...
    assert!(
        stdout.contains("statefulmemory prior context") && stdout.contains("pgx"),
...
fn read_hook_writes_only_stderr() {
    ...
    let _ = env
        ...
        .args([
            ...
            "fix",
            "--title",
            "write race in handler",
            "--content",
            "Fixed concurrent map write in src/server/handler.rs",
            "--session",
            &session,
        ])
        .output()
    ...
    let out = env
        ...
        .args([
            ...
            "Read",
            "--path",
            "crates/statefulmemory-storage/src/write.rs",
        ])
        .output()
    ...
    // "write"). Naive keyword extraction yields "write" + "statefulmemory" or
    // "storage" depending on stopword behavior; either should match the
    // saved fix.
    assert!(
        stderr.contains("statefulmemory hint") || stderr.is_empty(),
...
fn install_patches_pretool_grep_read_idempotent() {
    ...
    let statefulmemory_grep_count = pretool
        .iter()
        .filter(|b| {
                ...
                && b["hooks"]
                    ...
                    .map(|h| {
                        h.iter().any(|x| {
                            x["command"]
                                .as_str()
                                .map(|s| s.starts_with("statefulmemory "))
                                .unwrap_or(false)
                        })
                    })
                    .unwrap_or(false)
        ...
        .count();
    let statefulmemory_read_count = pretool
        .iter()
        .filter(|b| {
        ...
        .count();
    assert_eq!(statefulmemory_grep_count, 1, "exactly one Grep entry expected");
    assert_eq!(statefulmemory_read_count, 1, "exactly one Read entry expected");
