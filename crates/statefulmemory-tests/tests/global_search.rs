...
fn save(
    ...
    start
        .env("STATEFULMEMORY_PROJECT", project)
    ...
    let mut cmd = env.cmd();
    cmd.env("STATEFULMEMORY_PROJECT", project);
    let mut args = vec![
        "obs".to_string(),
        "save".to_string(),
        "--type".to_string(),
...
fn all_projects_returns_merged_results_with_project_tags() {
    ...
    let out = env
        .cmd()
        .env("STATEFULMEMORY_PROJECT", "repo-a")
        .args([
            "obs",
            "search",
            "kafka OR pgx",
...
fn same_topic_key_across_projects_kept_distinct() {
    ...
    assert!(
        b_count >= 1,
        "repo-b auth obs missing — same topic_key collapsed across projects"
    );
