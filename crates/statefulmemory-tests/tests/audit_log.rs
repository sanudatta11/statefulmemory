...
fn metadata_mode_logs_required_keys() {
    ...
    for line in &lines {
        ...
        assert!(
            line.get("duration_ms").is_some(),
            "missing duration_ms: {line}"
        );
    ...
    assert!(
        cmds.contains(&"obs.search"),
        "missing obs.search in {cmds:?}"
    );
...
// ---------------------------------------------------------------------------
// SC-7: STATEFULMEMORY_AUDIT_FULL=1 adds query + top_hits
// ---------------------------------------------------------------------------
...
fn full_mode_adds_query_and_top_hits() {
    ...
    let _ = env
        .cmd()
        .env("STATEFULMEMORY_AUDIT_FULL", "1")
...
fn unwritable_log_does_not_break_save() {
    ...
    let out = env
        ...
        .args([
            ...
            "save",
            "--title",
            "save-despite-bad-log",
            "--content",
            "the save itself must succeed",
            "--session",
            &session,
        ])
        .output()
