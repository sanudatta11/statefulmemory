...
fn ts7_save_update_get_bumps_revision() {
    ...
    let _updated: Value = json_cmd(
        &env,
        &[
            "--output", "json", "obs", "update", &id, "--title", "second",
        ],
    );
...
fn ts8_soft_delete_hides_hard_delete_removes() {
    ...
    assert!(
        !out.status.success(),
        "soft-deleted obs should not be retrievable: {:?}",
        out
    );
    ...
    assert!(
        !out.status.success(),
        "hard-deleted obs should not be retrievable"
    );
}
...
fn ts9_context_returns_recent_three() {
    ...
    let v: Value = json_cmd(
        &env,
        &["--output", "json", "obs", "context", "--limit", "10"],
    );
    let recents = v["recent_observations"]
        .as_array()
        .expect("recent_observations array");
    assert!(
        recents.len() >= 3,
...
fn ts19_due_for_review_filter() {
    ...
    let saved: Value = json_cmd(
        ...
        &[
            ...
            "use postgres",
            "--content",
            "decided to use postgres for the new service",
            "--session",
            &session,
        ],
    );
    let sync_id = saved["observation"]["sync_id"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    let v: Value = json_cmd(
        &env,
        &["--output", "json", "obs", "list", "--due-for-review"],
    );
    ...
    assert!(
        observations
            .iter()
            .all(|o| o["sync_id"].as_str() != Some(&sync_id)),
    ...
    assert!(
        observations
            .iter()
            .any(|o| o["sync_id"].as_str() == Some(&sync_id)),
...
fn ts20_suggest_topic_key_non_collision() {
    ...
    assert_ne!(
        key2, key,
        "second suggestion must avoid collision; got: {key2}"
    );
}
...
fn ts23_capture_passive_no_header_empty_list() {
    ...
    let out = env
        ...
        .args([
            ...
            "capture-passive",
            "--text",
            "Just some prose with no headers.\n",
            "--session",
            &session,
        ])
        .output()
