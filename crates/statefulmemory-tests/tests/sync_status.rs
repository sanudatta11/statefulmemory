...
async fn ts10_status_reads_manifest_length() {
    ...
    fs::create_dir_all(repo_root.path().join(".statefulmemory")).unwrap();
    ...
    fs::write(
        repo_root.path().join(".statefulmemory").join("manifest.json"),
    ...
    let mut client = statefulmemory_proto::stateful_memory_client::StatefulMemoryClient::new(channel);
