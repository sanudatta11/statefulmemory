...
use statefulmemory_tests::{connect, spawn_daemon, CliEnv};
...
async fn ts1_export_writes_valid_chunk() {
    ...
    let cid = resp
        .chunk_id
        .expect("chunk_id must be present for non-empty export");
    ...
    assert!(
        chunk_path.metadata().unwrap().len() > 0,
        "chunk file is empty"
    );
    ...
    for line in &lines {
        ...
        assert!(
            v.get("_kind").is_some(),
            "missing _kind discriminator: {line}"
        );
    }
    ...
    let manifest_path = repo_dir.path().join(".statefulmemory").join("manifest.json");
...
async fn ts2_export_idempotent() {
    ...
    let chunks_dir = repo_dir.path().join(".statefulmemory").join("chunks");
    ...
    assert_eq!(
        ...
        1,
        "expected exactly one chunk after first export"
    );
...
async fn ts13_export_all_no_cross_contamination() {
    ...
    for repo in [&repo_a, &repo_b, &repo_c] {
        let chunks_dir = repo.path().join(".statefulmemory").join("chunks");
    ...
    assert_eq!(
        unique.len(),
        3,
        "chunk_ids must be distinct across projects: {:?}",
        all_chunk_ids
    );
}
...
async fn connect_cli(
    env: &CliEnv,
) -> statefulmemory_proto::stateful_memory_client::StatefulMemoryClient<tonic::transport::Channel> {
    ...
    .await
    .map(statefulmemory_proto::stateful_memory_client::StatefulMemoryClient::new)
