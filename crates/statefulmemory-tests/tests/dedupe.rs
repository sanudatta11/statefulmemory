...
async fn start_session(
    client: &mut statefulmemory_proto::stateful_memory_client::StatefulMemoryClient<tonic::transport::Channel>,
    project: &str,
    id: &str,
) {
    client
...
async fn ts5_sync_id_idempotency() {
    ...
    assert_eq!(
        obs.title, "a",
        "sync_id idempotency: original row returned unchanged"
    );
