...
async fn ts19_concurrent_export_no_duplicate_rows() {
    ...
    let make_client = || {
        ...
        async move {
            ...
            let ch = ep
                ...
                .await
                .expect("connect");
            statefulmemory_proto::stateful_memory_client::StatefulMemoryClient::new(ch)
        }
    };
    ...
    let winning = match (r1.chunk_id.is_some(), r2.chunk_id.is_some()) {
        ...
        (true, true) => {
            ...
            assert_eq!(
                total, 5,
                "combined should equal total seeded rows: got {total}"
            );
    ...
    assert_eq!(
        files.len(),
        1,
        "exactly one chunk must exist after concurrent export"
    );
}
