...
//! global behaviors of the `statefulmemory` CLI binary.

use std::process::Stdio;

use statefulmemory_tests::{start_session, CliEnv};
...
fn ts4_piped_default_is_json() {
    ...
    assert!(
        ...
        "JSON missing `version` field: {stdout}"
    );
}
...
fn ts4_text_output_is_not_json() {
    ...
    assert!(
        ...
        "text mode should mention `version`: {stdout}"
    );
}
...
//
// statefulmemory never emits ANSI in JSON mode and only colors text-mode output
...
fn ts6a_explicit_project_flag_wins() {
    ...
    // Use cmd_no_project_env so STATEFULMEMORY_PROJECT doesn't shadow the flag.
    let out = env
        .cmd_no_project_env()
...
#[test]
fn ts6b_statefulmemory_project_env_used() {
    // SC-8 (-ish): STATEFULMEMORY_PROJECT env in absence of a --project flag.
    ...
    assert!(
        ...
        "STATEFULMEMORY_PROJECT should set the current project; got: {stdout}",
    );
}
...
fn ts6c_non_git_dir_returns_exit5() {
    ...
    let mut env = CliEnv::new();
    env.spawn_daemon();
    // No --project flag, no STATEFULMEMORY_PROJECT env, working dir is the tempdir
    // (not a git repo).
    let out = env
        .cmd_no_project_env()
        .args(["session", "list"])
...
// SC-7: `<cwd>/.statefulmemory/config.json` with `{"project_name": "X"}` takes
...
fn ts6_sc7_config_json_drives_project_name() {
    ...
    // valid, and put .statefulmemory/config.json there so detection finds it.
    let work_dir = env.data_path().join("workspace-sc7");
    let dot_dir = work_dir.join(".statefulmemory");
...
fn ts6d_project_flag_overrides_env() {
    // Cross-check: --project beats STATEFULMEMORY_PROJECT.
    let mut env = CliEnv::new().with_project("env-proj");
    env.spawn_daemon();
    let out = env
        .cmd()
    ...
    let stdout = String::from_utf8(out.stdout).unwrap();
    assert!(
        stdout.contains("flag-proj"),
        "flag should win; got: {stdout}"
    );
    assert!(
        !stdout.contains("env-proj"),
        "env should lose; got: {stdout}"
    );
}
...
//
// Spec: `statefulmemory [0-9]+\.[0-9]+\.[0-9]+ \(commit [0-9a-f]+`
...
fn ts16_version_output_format() {
    let env = CliEnv::new();
    let out = env.cmd().arg("version").output().expect("version");
    assert!(out.status.success());
    let stdout = String::from_utf8(out.stdout).unwrap();
    let line = stdout.trim();

    let rest = line
        .strip_prefix("statefulmemory ")
        .unwrap_or_else(|| panic!("missing `statefulmemory ` prefix: {line:?}"));
