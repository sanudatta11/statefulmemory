...
fn ts12_consolidate_dry_run_and_prune() {
    ...
    let consolidated: Value = json_cmd(
        &env,
        &[
            "--project",
            "alpha-svc",
    ...
    let names_after: Vec<&str> = after["projects"]
        ...
        .collect();
    assert!(
        names_after.contains(&"alpha-svc"),
        "alpha-svc must survive prune"
    );
    assert!(
        !names_after.contains(&"gamma"),
        "gamma should have been removed"
    );
    assert!(
        !names_after.contains(&"alpha-srv"),
        "alpha-srv should have been removed"
    );
}
...
fn ts24a_project_self_merge_invalid_argument() {
    ...
    let out = env
        .cmd()
        .args(["project", "merge", "--from", "loop", "--to", "loop"])
        .output()
