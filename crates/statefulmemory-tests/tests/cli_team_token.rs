...
//! unit test in `statefulmemory-daemon/src/service.rs::tests::create_token_logs_name_only_not_secret`,
...
fn ts14_token_create_then_list_no_secret_leak() {
    ...
    let listed: Value = json_cmd(&env, &["--output", "json", "team", "token-list"]);
...
// path directly via the `statefulmemory-client` library. This is still an
// integration test: a real daemon process listens on a real TLS socket, and
// the auth + admin-guard interceptors run in-process.
// ---------------------------------------------------------------------------

const ADMIN_TOKEN: &str = "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef";
const NON_ADMIN_TOKEN: &str = "2222222222222222222222222222222222222222222222222222222222222222";
...
async fn ts15a_tcp_valid_token_succeeds() {
    ...
    let channel = statefulmemory_client::channel::connect_tcp(&d.endpoint(), &d.ca_pem, "localhost")
        .await
        .expect("tcp channel");
    let interceptor = statefulmemory_client::interceptor::bearer_interceptor(ADMIN_TOKEN.to_string());
    let mut client = statefulmemory_client::StatefulMemoryClient::with_interceptor(channel, interceptor);
    let s = client
...
async fn ts15b_tcp_invalid_token_unauthenticated() {
    ...
    let channel = statefulmemory_client::channel::connect_tcp(&d.endpoint(), &d.ca_pem, "localhost")
        .await
        .expect("tcp channel");
    let interceptor = statefulmemory_client::interceptor::bearer_interceptor(WRONG_TOKEN.to_string());
    let mut client = statefulmemory_client::StatefulMemoryClient::with_interceptor(channel, interceptor);
    let err = client
        .daemon_status(DaemonStatusRequest {})
        .await
...
async fn ts15c_tcp_non_admin_daemon_stop_permission_denied() {
    ...
    let channel = statefulmemory_client::channel::connect_tcp(&d.endpoint(), &d.ca_pem, "localhost")
        .await
        .expect("tcp channel");
    let interceptor = statefulmemory_client::interceptor::bearer_interceptor(NON_ADMIN_TOKEN.to_string());
    let mut client = statefulmemory_client::StatefulMemoryClient::with_interceptor(channel, interceptor);
    let err = client
