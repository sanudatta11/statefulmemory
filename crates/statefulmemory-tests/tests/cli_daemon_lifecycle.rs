...
//
// Approach: set `STATEFULMEMORY_LISTEN=tcp://127.0.0.1:0` without `STATEFULMEMORY_TLS_*`.
...
fn ts2_unreachable_daemon_exits_4_quickly() {
    ...
    let out = env
        ...
        .env("STATEFULMEMORY_LISTEN", "tcp://127.0.0.1:0")
        .env_remove("STATEFULMEMORY_TLS_CERT")
        .env_remove("STATEFULMEMORY_TLS_KEY")
        .args(["obs", "recent"])
        .output()
        .expect("obs recent");
