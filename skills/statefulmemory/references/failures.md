...
Symptom: on your first turn there is no statefulmemory briefing in your context
...
statefulmemory obs context --limit 20
...
## CLI not installed (`command not found: statefulmemory`)
...
Building the binary only puts `statefulmemory` on PATH; it does not touch Claude
...
2. **If the sandbox blocks `git clone` / `cargo` / network** (managed
   ...
   ```bash
   !git clone https://github.com/sanudatta11/statefulmemory ~/Documents/statefulmemory && cd ~/Documents/statefulmemory && cargo build --release -p statefulmemory-cli && mkdir -p ~/.local/bin && ln -sf "$PWD/target/release/statefulmemory" ~/.local/bin/statefulmemory && export PATH="$HOME/.local/bin:$PATH"
   ```

3. **If the repo is already on disk somewhere** (`~/Documents/statefulmemory`,
   `~/code/statefulmemory`, `~/src/statefulmemory`, etc.), skip the clone — just
   build and symlink.

4. **If the user can't or won't install**, warn **once** per session:
   "statefulmemory unavailable — memory persistence is OFF for this session."
...
- Likely a stale socket (the daemon crashed and left `daemon.sock` behind).
  ...
  ```bash
  statefulmemory daemon force-start
  ```

## Project name could not be determined (exit code 5)

statefulmemory auto-detects from `git remote origin` then git-root basename.
If both fail (e.g. not in a git repo), exit 5 fires.

- Tell the user to set `STATEFULMEMORY_PROJECT`:
  ```bash
  export STATEFULMEMORY_PROJECT=my-project
  ```
...
```bash
STATEFULMEMORY_AUDIT_FULL=1 statefulmemory obs context --query "<the query that missed>"
tail -5 ~/.statefulmemory/queries.log | jq .
```
...
```bash
statefulmemory obs context --query "<the query that missed>" --mode hybrid
```
...
```bash
statefulmemory obs context --query "..." --mode hybrid --rerank haiku
```

## Sandbox blocks the daemon socket

If the agent runs in a restricted sandbox, the Unix socket may be denied
(`Operation not permitted`). The user must allow `~/.statefulmemory/daemon.sock`
