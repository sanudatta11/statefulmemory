# statefulmemory — driving memory from the CLI (no hooks, no settings changes)
...
On your first turn, look for a statefulmemory briefing in your context —
...
```bash
echo "## Goal\n...\n## Done\n- ..." \
    | statefulmemory session summarize "$SESSION_ID" --content -
```
...
```bash
statefulmemory obs context --query "<topic>" --limit 10
```
