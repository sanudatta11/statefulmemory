...
- **Title is the search key.** Make it specific enough that running
  `statefulmemory obs search "<topic>"` three months from now will surface it.
...
```bash
statefulmemory obs search "<keyword>" --limit 10
statefulmemory obs search "<keyword>" --type fix
```
