# statefulmemory — `/statefulmemory <text>` slash command

When the user types `/statefulmemory <anything>`, asks to "save to statefulmemory", or
...
|---|---|---|---|
| `/statefulmemory my project deadline is 20 June` | note | "project deadline: 20 June 2026" | "Project deadline: 20 June 2026 (per user)" |
| `/statefulmemory prefer pgx over GORM, codegen overhead is a no-go` | decision | "use pgx, not GORM" | "Team prefers raw SQL via pgx; GORM rejected for codegen overhead" |
...
| `save that as a statefulmemory observation` (after fixing nil-deref) | fix | "nil-deref in handler X caused by uninit map" | "Bug fix: handler X assumed map was initialized; uninit map caused nil-deref. Fix: initialize map in constructor." |

## Rules

- **No confirmation step.** Save immediately. The user already asked.
- **No fallback to built-in memory.** If `statefulmemory` CLI is unavailable,
