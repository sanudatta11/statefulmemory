# statefulmemory — concrete examples
...
statefulmemory obs search "http client" --limit 10

# Before designing a new error type:
statefulmemory obs search "error" --type pattern --limit 5
```
...
```bash
statefulmemory obs context --query "rate limiting" --limit 10
```
...
```bash
statefulmemory obs recent --limit 10                    # all types
statefulmemory obs recent --limit 10 --type decision    # decisions only
statefulmemory obs get 42                               # full content for one
```
...
```bash
# Find the project DB:
ls ~/.statefulmemory/projects/

# Raw query:
sqlite3 ~/.statefulmemory/projects/<project>.sqlite \
...
```bash
statefulmemory obs search "X" --limit 5
```
