# statefulmemory — contributor notes for AI coding agents
...
The `statefulmemory` binary is a thin CLI (`crates/statefulmemory-cli`) that auto-spawns a
per-user gRPC daemon (`crates/statefulmemory-daemon`) on first call. The daemon owns
one SQLite+FTS5 per-project DB (`crates/statefulmemory-storage`), a cross-project
...
(`crates/statefulmemory-retrieval`). Optional time decay, evidence-window expansion,
and per-type quotas are config-gated (default off / unlimited). The wire
protocol is proto3 gRPC (`proto/statefulmemory.proto`).

## Crate dependency order (low → high)

```
statefulmemory-core              (config, paths, error, git, tokens)
statefulmemory-proto             (tonic-generated stubs)
statefulmemory-storage           (rusqlite, facts, anchors, conflict_judge trait)
statefulmemory-embed             (BgeSmallEmbedder, quantize)
statefulmemory-extract           (agent_cli, ClaudeClient, ClaudeCliExtractor)
statefulmemory-retrieval         (rrf_fuse / rrf_fuse_scored, ClaudeReranker, HybridMode)
statefulmemory-daemon            (service, verify, verify_worker, context_filter, token_budget, …)
statefulmemory-client            (channel builders: UDS + TCP)
statefulmemory-cli               (statefulmemory binary; install / git_hooks / mcp)
statefulmemory-sync              (export/import, .mem archives)
statefulmemory-mcp               (stdio MCP server)
statefulmemory-eval              (benchmark harness — not imported by daemon)
...
- **Write thread** per project: `storage::write::spawn_write_thread`. All DB
  ...
  stamp).
- **Config** (`StatefulMemoryConfig`) uses 3-level TOML merge: env vars >
  `~/.statefulmemory/projects/<name>.config.toml` > `~/.statefulmemory/config.toml` >
...
- **Git** helpers shell out to `git` (`statefulmemory-core::git`); no git2/gix.
- **Tests**: integration tests in `crates/statefulmemory-tests/` require a live
...
BIN=$(find target/debug/deps -maxdepth 1 -name 'statefulmemory_storage-*' -perm -u+x | tail -1)
...
#   STATEFULMEMORY_LLM_PROVIDER=opencode STATEFULMEMORY_LLM_BIN=opencode \
#   STATEFULMEMORY_LLM_MODEL=opencode-go/deepseek-v4.1-flash \
#   STATEFULMEMORY_EVAL_CONCURRENCY=4 LIMIT=50 make eval-locomo
...
## Config quick-reference (StatefulMemoryConfig)

```toml
# ~/.statefulmemory/config.toml (global)
# ~/.statefulmemory/projects/<name>.config.toml (per-project overlay)
...
`STATEFULMEMORY_LLM_MODEL` or a concrete id (`qwen`, `opencode/glm-5.3`). Host hints:
...
Doctor RPCs — see `proto/statefulmemory.proto`.
...
- Spec 2 (shipped): MCP — `statefulmemory mcp` + seven `memory_*` tools; install
  targets Claude Code, Cursor, Windsurf, Antigravity, OpenCode, Kimi Code,
  ZCode, VS Code / Copilot, Codex, Gemini CLI, Amazon Q, `.agents`
- Spec 3 (partial): anchors + `statefulmemory verify` + stale withdrawal + git hooks
...
- Spec 1 deferred items (shipped): int8 quantize, cross-project hybrid,
  `obs reextract`, `statefulmemory reindex`
- `statefulmemory-eval` CI scorecard wiring (separate `eval-promotion` spec)
